...
void LightningShaderGenerator::InitializeSpirV()
{
  ...
  EventConnect(&mFragmentsProject, Lightning::Events::CompilationError, &LightningShaderGenerator::OnLightningFragmentCompilationError, this);
  EventConnect(&mFragmentsProject, Lightning::Events::TypeParsed, &LightningShaderGenerator::OnLightningFragmentTypeParsed, this);
  EventConnect(&mFragmentsProject, Events::TranslationError, &LightningShaderGenerator::OnLightningFragmentTranslationError, this);
  EventConnect(&mFragmentsProject, Events::ValidationError, &LightningShaderGenerator::OnLightningFragmentValidationError, this);
}
...
LibraryRef BuildWrapperLibrary(LightningShaderIRLibraryRef fragmentsLibrary)
{
  ...
  // The Plasma editor attributes to check for valid crossover attributes from the shader types.
  ...
  forRange (LightningShaderIRType* shaderType, fragmentsLibrary->mTypes.Values())
  {
    ...
    if (shaderTypeMeta->mFragmentType != FragmentType::None)
    {
      ...
      BoundType* boundType = builder.AddBoundType(shaderTypeMeta->mLightningName, TypeCopyMode::ReferenceType, fragmentSize);
      boundType->BaseType = LightningTypeId(MaterialBlock);
      // Associate this type to the LightningFragment resource containing the shader type.
      ...
      forRange (ShaderPropertyInfo& shaderProperty, shaderProperties.All())
      {
        ...
        if (shaderProperty.mIsTexture)
        {
          ...
          if (texture != nullptr)
            *(u64*)defaultElement = (u64)texture->mResourceId.mValue;
        }
        else
        {
...
//**************************************************************************************************
LibraryRef LightningShaderGenerator::BuildFragmentsLibrary(Module& dependencies, Array<LightningDocumentResource*>& fragments, StringParam libraryName)
{
  ...
  LightningShaderIRLibraryRef fragmentsLibrary = mFragmentsProject.CompileAndTranslate(internalDependencies, mFrontEndTranslator);
  ...
  forRange (LightningShaderIRType* shaderType, fragmentsLibrary->mTypes.Values())
  {
    ...
    // Not all types have backing lightning types either (runtime arrays have an internal type with no backing lightning type)
    Lightning::BoundType* lightningType = shaderTypeMeta->mLightningType;
    if(lightningType == nullptr)
      continue;

    Resource* resource = (Resource*)lightningType->Location.CodeUserData;
...
//**************************************************************************************************
bool LightningShaderGenerator::BuildShaders(ShaderSet& shaders, HashMap<String, UniqueComposite>& composites, Array<ShaderEntry>& shaderEntries, Array<ShaderDefinition>* compositeShaderDefs)
{
  ...
  for (size_t startIndex = 0; startIndex < totalShaderCount; startIndex += compositeBatchCount)
  {
    ...
    for (size_t i = startIndex; i < endIndex; ++i)
    {
      ...
      // Used for debugging purposes to display the lightning composites.
    ...
    EventConnect(&shaderProject, Lightning::Events::CompilationError, &LightningShaderGenerator::OnLightningFragmentCompilationError, this);
...
//**************************************************************************************************
bool LightningShaderGenerator::CompilePipeline(LightningShaderIRType* shaderType, ShaderPipelineDescription& pipeline, Array<TranslationPassResultRef>& pipelineResults)
{
...
//**************************************************************************************************
ShaderInput LightningShaderGenerator::CreateShaderInput(StringParam fragmentName, StringParam inputName, ShaderInputType::Enum type, AnyParam value)
{
  ...
  // the LightningShaderGenerator does not change its setup and naming schemes.
  ...
  else
  {
    ...
    // SPIR-V doesn't allow boolean uniforms so we convert boolean inputs from Lightning to integers.
...
void LightningShaderGenerator::OnLightningFragmentCompilationError(Lightning::ErrorEvent* event)
{
  ...
  LightningFragmentManager::GetInstance()->DispatchScriptError(Events::SyntaxError, shortMessage, fullMessage, event->Location);
}

//**************************************************************************************************
void LightningShaderGenerator::OnLightningFragmentTypeParsed(Lightning::ParseEvent* event)
{
  // There are a lot of attributes in lightning fragments that aren't valid for lightning script.
...
{
  String fullMessage = event->GetFormattedMessage(Lightning::MessageFormat::Python);
  LightningFragmentManager::GetInstance()->DispatchScriptError(Events::SyntaxError, event->mShortMessage, fullMessage, event->mLocation);
}
