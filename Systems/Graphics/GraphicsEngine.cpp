...
void GraphicsEngine::StartProgress(Event* event)
{
  ...
  {
      DoNotifyWarning("Graphics", "Unable to find startup textures");
      return; 
  }
...
void GraphicsEngine::CheckTextureYInvert(Texture* texture)
{
  ...
  // Uv coordinate correction from Plasma to Api is done by the shader translation of texture samples
  if (!PL::gRenderer->YInvertImageData(texture->mType))
    return;

  // All incoming image data from Plasma should be a color format and/or block compressed
...
{
  return mShaderGenerator->mFragmentTypes.FindValue(LightningVirtualTypeId(materialBlock)->Name, LightningFragmentType::Fragment);
}
...
void GraphicsEngine::ModifiedFragment(LightningFragmentType::Enum type, StringParam name)
{
  switch (type)
  {
    case Plasma::LightningFragmentType::CoreVertex: mModifiedCoreVertex.PushBack(name); break;
    case Plasma::LightningFragmentType::RenderPass: mModifiedRenderPass.PushBack(name); break;
    case Plasma::LightningFragmentType::PostProcess: mModifiedPostProcess.PushBack(name); break;
  }
}
...
void GraphicsEngine::RemovedFragment(LightningFragmentType::Enum type, StringParam name)
{
  switch (type)
  {
    case Plasma::LightningFragmentType::CoreVertex: mRemovedCoreVertex.PushBack(name); break;
    case Plasma::LightningFragmentType::RenderPass: mRemovedRenderPass.PushBack(name); break;
    case Plasma::LightningFragmentType::PostProcess: mRemovedPostProcess.PushBack(name); break;
  }
}
...
void GraphicsEngine::OnScriptsCompiledPrePatch(LightningCompileEvent* event)
{
  forRange (ResourceLibrary* modifiedLibrary, event->mModifiedLibraries.All())
  {
    ...
    LightningFragmentTypeMap& currentFragmentTypes =
    ...
    if (currentLibrary != nullptr)
    {
      forRange (LightningShaderIRType* shaderType, currentLibrary->mTypes.Values())
      {
        ...
        LightningFragmentType::Enum fragmentType = currentFragmentTypes.FindValue(shaderTypeMeta->mLightningName, LightningFragmentType::Fragment);
    ...
    if (mModifiedFragmentFiles.Empty() == false)
    {
      forRange (LightningShaderIRType* shaderType, pendingLibrary->mTypes.Values())
      {
        ...
        if (mModifiedFragmentFiles.Contains(resourceName))
        {
          ...
          LightningFragmentType::Enum currentFragmentType = currentFragmentTypes.FindValue(shaderTypeMeta->mLightningName, LightningFragmentType::Fragment);
          LightningFragmentType::Enum pendingFragmentType = pendingFragmentTypes.FindValue(shaderTypeMeta->mLightningName, LightningFragmentType::Fragment);
          ...
          forRange (LightningShaderIRType* dependent, dependents.All())
          {
            ...
            LightningFragmentType::Enum dependentType = pendingFragmentTypes.FindValue(dependentTypeMeta->mLightningName, LightningFragmentType::Fragment);
