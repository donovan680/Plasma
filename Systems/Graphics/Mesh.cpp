...
LightningDefineType(VertexBuffer, builder, type)
{
  ...
  LightningBindOverloadedMethod(GetVertexData, LightningInstanceOverload(Vec4, uint, VertexSemantic::Enum, VertexElementType::Enum, uint));
...
void LoadIndexChunk(Mesh& mesh, streamType& file)
{
  ...
  switch (indexType)
  {
  case Plasma::IndexElementType::Byte:   FillIndexBuffer<byte>(indexBuffer, indexBufferData, numIndicies);
    break;
  case Plasma::IndexElementType::Ushort: FillIndexBuffer<ushort>(indexBuffer, indexBufferData, numIndicies);
    break;
  case Plasma::IndexElementType::Uint:   FillIndexBuffer<uint>(indexBuffer, indexBufferData, numIndicies);
    break;
  }
