...
LightningDefineType(Camera, builder, type)
{
  ...
  LightningBindGetterSetterProperty(FieldOfView)->Add(new EditorSlider(45, 135, 1))->PlasmaFilterEquality(mPerspectiveMode, PerspectiveMode::Enum, PerspectiveMode::Perspective);
...
Mat4 Camera::GetPerspectiveTransform()
{
  ...
  {
    BuildPerspectiveTransformPlasma(mViewToPerspective, Math::DegToRad(mFieldOfView), mAspectRatio, mNearPlane, mFarPlane);
    PL::gRenderer->BuildPerspectiveTransform(mViewToApiPerspective, Math::DegToRad(mFieldOfView), mAspectRatio, mNearPlane, mFarPlane);
  }
  else
  {
