...
bool SoundNode::Evaluate(BufferType* outputBuffer, const unsigned numberOfChannels, ListenerNode* listener)
{
  ...
  if (PL::gSound->Mixer.mMixVersionThreaded == mMixedVersionThreaded)
  {
    ...
    if (!message.Empty())
    {
      ...
      PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundNode::WarningFromMixThread, this,
        title, message), this);

      PL::gSound->Mixer.AddTaskThreaded(CreateFunctor(&SoundNode::RemoveAndAttachInputsToOutputs, 
        this), this);
...
//**************************************************************************************************
CombineNode::CombineNode(Plasma::StringParam name, unsigned ID) :
