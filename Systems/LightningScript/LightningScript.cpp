...
//**************************************************************************************************
// @TrevorS: Isn't this the same logic as AddDependencies on ResourceLibrary/LightningManager?
    ...
    mLastExceptionVersion(-1)
{
  ...
  // We want LightningScript to be the first thing that shows up in the "Code" category in the add window
  mAddSortWeight = 0;
  ...
  Lightning::EventConnect(ExecutableState::CallingState, Lightning::Events::UnhandledException, PlasmaLightningExceptionCallback);
...
void LightningScriptManager::ValidateRawName(Status& status, StringParam name, BoundType* optionalType)
{
  if (!optionalType || optionalType->IsA(LightningTypeId(Component)))
  {
    ...
    bool hasMember =
      LightningTypeId(Cog)->GetMember(name) ||
      LightningTypeId(GameSession)->GetMember(name) ||
      LightningTypeId(Space)->GetMember(name) ||
      LightningTypeId(CogPath)->GetMember(name);
...
void LightningScriptManager::DispatchScriptError(StringParam eventId, StringParam shortMessage, StringParam fullMessage, const CodeLocation& location)
{
...
void LightningScriptManager::DispatchPlasmaLightningError(const CodeLocation& location, StringParam message, Project* buildingProject)
{
  String shortMessage = BuildString("Plasma Error: ", message);
