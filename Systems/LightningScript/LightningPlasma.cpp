...
LightningDefineType(LightningComponent, builder, type)
{
  ...
  // Temporary solution so that LightningComponent cannot be added on Cog in the property grid
...
void LightningComponent::DebugDraw()
{
  ...
  Lightning::Function* function = thisType->FindFunction(FunctionName, Array<Lightning::Type*>(),
                                                     core.VoidType, Lightning::FindMemberOptions::None);

  // Do not want to re-invoke Component's DebugDraw, will not find LightningComponent's DebugDraw because it is not bound
...
LightningDefineType(LightningEvent, builder, type)
{
  ...
  // If LightningEvent's created in Lightning were using the same handle manager as C++ Events, they
  // would leak. So, we want just LightningEvents to be reference counted. We're using HeapManager
  ...
  // C++ to Lightning (because of HeapManager), and this should never be constructed in C++.
