  ...
  else if(value.Is<Any>())
  {
    ...
    else if(anyValue.StoredType->IsA(LightningTypeId(Resource)))
    {
      ...
      if(direction == SerializeDirection::Write)
      {
        ...
        // Get resource ID
        u64 resourceId = (u64)(resource ? ((Guid)resource->mResourceId).mValue : ResourceId(0).mValue);
...
/// Returns true if BitStream can serialize the specified lightning type, else false.
inline bool BitStreamCanSerializeType(Type* lightningType)
{
  ...
  return (basicNativeType != nullptr)
      || (lightningType->IsEnum())
      || (lightningType->IsA(LightningTypeId(Resource)))
