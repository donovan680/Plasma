...
bool NetPeer::SendLevelLoadStarted(NetPeerId theirNetPeerId, Space* space, Level* level)
{
  ...
  netLevelLoadStartedData.mLevelResourceId  = (u64)(level ? level->mResourceId.mValue : ResourceId(0).mValue);
...
bool NetPeer::ReceiveLevelLoadStarted(NetPeerId theirNetPeerId, const IpAddress& theirIpAddress, const Message& message)
{
  ...
  {
    // Get level resource
    Level* level = static_cast<Level*>(PL::gResources->GetResource((ResourceId)netLevelLoadStartedData.mLevelResourceId));
    if(!level) // Unable?
    {
...
Pair<bool, BitStream> NetPeer::ServerOnConnectRequest(ReplicatorLink* link, ConnectRequestData& connectRequestData)
{
  ...
  Pair<bool, BitStream> result(event.mReturnOurConnectResponse, PlasmaMove(event.mReturnOurResponseBundle.GetBitStream()));
