...
// Sent out on the PL::gResources in the constructor of ResourceLibrary (no resources added/loaded yet).
...
//--------------------------------------------------------------------- Lightning Compile Fragment Event
...
public:
  ...
  LightningCompileFragmentEvent(Module& dependencies, Array<LightningDocumentResource*>& fragments, ResourceLibrary* owningLibrary);
...
/// EventHandler is Lightning's EventObject and is needed for compilation events. This should be
/// removed once we combine the two
class ResourceLibrary : public EventObject, public EventHandler
{
public:
  LightningDeclareType(ResourceLibrary, TypeCopyMode::ReferenceType);
  ...
  /// the LightningManager compile events before libraries are committed.
  ...
  // during the Graphics and LightningScript project initialization.
