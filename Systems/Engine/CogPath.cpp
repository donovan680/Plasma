...
Cog* CogPath::RestoreLink(CogInitializer& initializer, Cog* owner, Component* component, StringParam propertyName)
{
  ...
  Plasma::RestoreLink(&mSharedNode->mResolvedCog, initializer.Context, component, propertyName, GetErrorOnDirectLinkFail());
...
namespace Serialization
{
  bool Policy<CogPath>::Serialize(Serializer& stream, cstr fieldName, CogPath& path)
  {
    ...
    if (stream.Start("CogPath", fieldName, StructureType::Object))
    {
      ...
      if (!stream.EnumField("CogPathPreference", "PathPreference0", (uint&)value.mPathPreference0, LightningTypeId(CogPathPreference::Enum)))
        value.mPathPreference0 = CogPathPreference::CogRelative;
      if (!stream.EnumField("CogPathPreference", "PathPreference1", (uint&)value.mPathPreference1, LightningTypeId(CogPathPreference::Enum)))
        value.mPathPreference1 = CogPathPreference::SpaceRelative;
      if (!stream.EnumField("CogPathPreference", "PathPreference2", (uint&)value.mPathPreference2, LightningTypeId(CogPathPreference::Enum)))
...
CogPathMetaComposition::CogPathMetaComposition()
  : MetaComposition(LightningTypeId(Component))
{
...
Handle CogPathMetaComposition::GetComponent(HandleParam owner, BoundType* componentType)
{
  ...
  if (Cog* cog = cogPath->GetCog())
  {
    ...
    // the actual type we were querying. This can cause a problem when building a Lightning::Call.
