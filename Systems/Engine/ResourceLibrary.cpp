...
LightningCompileFragmentEvent::LightningCompileFragmentEvent(Module& dependencies,
  Array<LightningDocumentResource*>& fragments, ResourceLibrary* owningLibrary) :
...
SwapLibrary::SwapLibrary() :
  mCompileStatus(LightningCompileStatus::Modified)
{
}
...
ResourceLibrary::ResourceLibrary()
{
  ...
  LightningManager::GetInstance()->mDebugger.AddProject(&mScriptProject);
...
ResourceLibrary::~ResourceLibrary()
{
  LightningManager::GetInstance()->mDebugger.RemoveProject(&mScriptProject);
}
...
{
  // Plugins should only depend on the Core library and native Plasma libraries (the only libraries we generate headers for)
