...
{
  // This should probably look through only LightningScripts for "class" and LightningFragments for "struct"
  // This can possibly point a proxied fragment to a LightningScript
  String regexString = String::Format("(class|struct)\\s+%s", proxiedType->Name.c_str());

  Regex regex(regexString);

  forRange(ResourceId textResourceId, PL::gResources->TextResources.Values())
  {
    DocumentResource* textResource = (DocumentResource*)PL::gResources->GetResource(textResourceId);
...
void EngineLibraryExtensions::AddResourceExtensions(LibraryBuilder& builder, ResourceLibrary* resources)
{
  ...
  forRange(HandleOf<Resource> resourceHandle, resources->Resources.All())
  {
    ...
    u64 resourceId = ((Guid)resource->mResourceId).mValue;
