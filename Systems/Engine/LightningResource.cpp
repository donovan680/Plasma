...
void LightningDocumentResource::FindPositionToGenerateFunction(ICodeEditor* editor, int& positionOut, String& indent)
{
  ...
  // The entire idea here is that we scanned up until we found the function we were calling Plasma.Connect in
...
void LightningDocumentResource::ValidateRawScriptName(Status& status, StringParam name)
{
  ...
  {
    status.SetFailed("Lightning type names must start with an uppercase letter and not contain invalid symbols");
    return;
  }
}
