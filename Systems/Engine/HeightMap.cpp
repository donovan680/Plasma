...
void HeightMap::SaveToHeightMapSource(Serializer& stream)
{
  ...
  if(PL::gRuntimeEditor)
    mSource = (HeightMapSource*)PL::gRuntimeEditor->NewResourceOnWrite(HeightMapSourceManager::GetInstance(), LightningTypeId(HeightMap), "Source", GetSpace(), mSource, archetype, mModified);
