...
set(PLASMA_CORE_ROOT "" CACHE STRING "Root folder of all of Plasma source")

set(plasmaBuildFolder ${PLASMA_CORE_ROOT}/Build)

set(buildVersionIdsFolder ${plasmaBuildFolder}/BuildVersionIds)

# file we are going to write to
set(tempFile ${plasmaBuildFolder}/BuildVersion.temp)

# check if git exists
execute_process(
    COMMAND git status > nul
    RESULT_VARIABLE procResult
    WORKING_DIRECTORY "${PLASMA_CORE_ROOT}"
)
...
if (${procResult} EQUAL 0)
    ...
    # line that will be inserted into temp file to define PlasmaRevisionId
    ...
    set(format          "#define PlasmaShortChangeSet %h%n")
...
#---------------------------------------------------------------NoSourceControl
    ...
    file(APPEND "${tempFile}" "PlasmaRevisionId 0\n")
    file(APPEND "${tempFile}" "#define PlasmaShortChangeSet 0\n")
    file(APPEND "${tempFile}" "PlasmaChangeSet 0 \n")
    file(APPEND "${tempFile}" "PlasmaChangeSetDate \"\"\n")
...
execute_process(
    ...
    "${tempFile}"
    # output
    "${PLASMA_CORE_ROOT}/Systems/Engine/BuildVersion.inl"
)
