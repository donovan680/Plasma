...
{
  DoNotifyException("Lightning Error", String::Format(
    "Plasma.Connect must take a delegate that has only one "
...
void PlasmaStatic::Connect(Object* target, StringParam eventId, DelegateParam delegate)
{
  ...
  LightningScriptConnection* connection = new LightningScriptConnection(delegate);
...
//**************************************************************************************************
LightningScriptConnection::LightningScriptConnection(DelegateParam delagate)
{
...
{
  // Since LightningScriptConnections contain a handle in the delegate, it is possible for the event
...
void LightningScriptConnection::RaiseError(StringParam message)
{
  ...
  if(location)
  {
    ...
    LightningScriptManager::GetInstance()->DispatchScriptError(Events::UnhandledException, namedMessage, fullMessage, *location);
    return;
  }
...
void LightningScriptConnection::Invoke(Event* e)
{
  ...
  // The Lightning binding will yell at us for trying to pass in an 'Event' where it takes a more derived type
