...
void ArchiveLibraryOutput(Archive& archive, ContentLibrary* library)
{
  ...
  forRange(ResourceEntry resource, listing.All())
  {
    ...
    PL::gEngine->LoadingUpdate("Archive Library", library->Name, resource.Name, ProgressType::Normal, float(itemsDone) / librarySize);
  }
    ...
    forRange(ResourceEntry& entry, listing.All())
    {
      ...
      if (isTemplate)
      {
        ...
        if (resourceType->IsA(lightningDocumentType)     ||
            resourceType->IsA(LightningPluginSourceType) ||
  ...
  : Composite(parent)
{
  ...
  mExportPath->SetText(FilePath::Combine(GetUserDocumentsDirectory(), "PlasmaExports", projectSettings->GetProjectName()));
