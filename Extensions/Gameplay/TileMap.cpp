...
{
  size_t hash = 0;
  hash ^= (size_t)ArchetypeResource.mValue;
  hash ^= (size_t)SpriteResource.mValue;
  hash ^= (size_t)CollisionResource.mValue;
  return hash;
}
...
void TileMap::SaveToTileMapSource(Serializer& stream)
{
  ...
  if(PL::gRuntimeEditor)
    mSource = (TileMapSource*)PL::gRuntimeEditor->NewResourceOnWrite(TileMapSourceManager::GetInstance(), LightningTypeId(TileMap), "Source", GetSpace(), mSource, archetype, mModified);
