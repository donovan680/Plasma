...
void EditText::RenderUpdate(ViewBlock& viewBlock, FrameBlock& frameBlock, Mat4Param parentTx, ColorTransform colorTx, WidgetRect clipRect)
{
  ...
  if (mEditEnabled && mHasFocus)
  {
    ...
    if (IsValidSelection())
    {
      ...
      pos1.y = mSize.y;
      boxColor = ToFloatColor(Color::DarkPurple);
    }
...
void EditText::OnKeyDown(KeyboardEvent* keyboardEvent)
{
  ...
  if(ctrlPressed)
  {
    switch(key)
    {
      ...
      case Keys::V:
      {
        String toPaste = PL::gEngine->has(OsShell)->GetClipboardText();
      ...
      case Keys::C:
      {
        ...
        PL::gEngine->has(OsShell)->SetClipboardText(toCopy);
        keyboardEvent->Handled = true;
        break;
      }
      ...
      case Keys::X:
      {
        ...
        PL::gEngine->has(OsShell)->SetClipboardText(toCut);
