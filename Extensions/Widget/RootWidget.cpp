...
void RootWidget::OnUiRenderUpdate(Event* event)
{
  ...
  HandleOf<RenderTarget> renderTarget = PL::gEngine->has(GraphicsEngine)->GetRenderTarget((uint)mSize.x, (uint)mSize.y, TextureFormat::RGBA8);
  ...
  {
    Pair<u64, uint> key((u64)spriteMaterial->mResourceId.mValue, shaderInputsId);
