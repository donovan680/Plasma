...
void DispatchCodeTranslatorScriptError(StringParam eventId, StringParam shortMessage, StringParam fullMessage, const Lightning::CodeLocation& location)
{
  // This should only happen when a composite has a lightning error. Figure out how to report later?
...
//void TranslatedShaderScriptEditor::CompileAndTranslateFragments(SimpleLightningShaderGenerator& shaderGenerator)
//{
...
//  EventConnect(&shaderGenerator, Events::TranslationError, &OnLightningFragmentTranslationError);
//  EventConnect(&shaderGenerator, Lightning::Events::CompilationError, &OnLightningFragmentCompilationError);
//
//  // Remove all old code
...
//  String settingsDir = FilePath::Combine(mainConfig->DataDirectory, "LightningFragmentSettings");
//  String extensionsDir = FilePath::Combine(mainConfig->DataDirectory, "LightningFragmentExtensions");
...
//  LightningFragmentManager::ResourceIdMapType::valuerange range = manager->ResourceIdMap.Values();
...
//}
//
//void TranslatedShaderScriptEditor::TranslateMaterial(SimpleLightningShaderGenerator& shaderGenerator, Material* material)
//{
...
//
//  // For now this assumes that all materials blocks on the material are lightning fragments
//  LightningShaderDefinition shaderDef;
...
//-------------------------------------------------------------------LightningCompositorScriptEditor
LightningCompositorScriptEditor::LightningCompositorScriptEditor(Composite* parent)
  : TranslatedShaderScriptEditor(parent)
{
  mMaterial = nullptr;
}
...
String LightningCompositorScriptEditor::OnTranslate()
{
  ...
  //// find which lightning file was created for the material. As this is not a normal operation in
...
String TranslatedLightningCompositorScriptEditor::OnTranslate()
{
  ...
  //  return "Failed to compile";
  //
  //LightningShaderDefinition* result = shaderGenerator.FindShaderResults(mMaterial->Name);
  //if(result == nullptr)
  //  return "Failed to find";
  //
  //String vertexShaderName = result->mShaderData[FragmentType::Vertex].mLightningClassName;
  //String geometryShaderName = result->mShaderData[FragmentType::Geometry].mLightningClassName;
  //String pixelShaderName = result->mShaderData[FragmentType::Pixel].mLightningClassName;
  //
...
void BaseSplitScriptEditor::OnLeftMouseDown(MouseEvent* e)
{
  ...
  //SimpleLightningShaderGenerator* shaderGenerator = mTranslatedEditor->mShaderGenerator;
  //LightningShaderProject& fragmentProject = shaderGenerator->mFragmentProject;
  //LightningShaderProject& shaderProject = shaderGenerator->mShaderProject;
  //for(size_t i = 0; i < mTranslatedEditor->mShaderGenerator->mFragmentProject.mCodeEntries.Size(); ++i)
  //{
  //  LightningShaderProject::CodeEntry& entry = mTranslatedEditor->mShaderGenerator->mFragmentProject.mCodeEntries[i];
...
//{
//  LightningShaderLibrary* library = mTranslatedEditor->mShaderGenerator->mFragmentLibraryRef;
...
//{
//  LightningShaderDefinition* def = mTranslatedEditor->mShaderGenerator->FindShaderResults(mMaterial->Name);
//
...
CodeTranslatorListener::CodeTranslatorListener()
{
  ...
  //ConnectThisTo(PL::gEditor, "TranslateLightningFragment", OnTranslateLightningFragmentWithLineNumbers);
  //ConnectThisTo(PL::gEditor, "TranslateLightningPixelMaterial", OnTranslateLightningPixelFragmentWithLineNumbers);
  //ConnectThisTo(PL::gEditor, "TranslateLightningVertexMaterial", OnTranslateLightningVertexFragmentWithLineNumbers);
}
...
void CodeTranslatorListener::OnComposeLightningMaterial(ObjectEvent* e)
{
  Editor* editor = PL::gEditor;
  Material* material = (Material*)e->Source;
  LightningCompositorScriptEditor* scriptEditor = CreateScriptWindow<LightningCompositorScriptEditor>(editor, BuildString("CompositedMat", material->Name), Lexer::Lightning);
  scriptEditor->SetResource(material);
  scriptEditor->Build();
}
...
{
  CreateMaterialTranslationWindow<TranslatedLightningCompositorScriptEditor>(e, TranslationDisplayMode::Pixel, "TranslatedPixelMat");
}

void CodeTranslatorListener::OnTranslateLightningGeometryFragment(ObjectEvent* e)
{
  CreateMaterialTranslationWindow<TranslatedLightningCompositorScriptEditor>(e, TranslationDisplayMode::Geometry, "TranslatedGeometryMat");
}

void CodeTranslatorListener::OnTranslateLightningVertexFragment(ObjectEvent* e)
{
  CreateMaterialTranslationWindow<TranslatedLightningCompositorScriptEditor>(e, TranslationDisplayMode::Vertex, "TranslatedVertexMat");
}
