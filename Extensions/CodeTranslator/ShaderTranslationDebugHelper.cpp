...
void FragmentSearchProvider::Search(SearchData& search, LightningShaderIRLibrary* shaderLibrary, HashSet<LightningShaderIRLibrary*>& visitedLibraries)
{
...
SearchView* ShaderTranslationDebugHelper::CreateFragmentSearchView(StringParam attribute)
{
  ...
  // Create a search view to filter lightning fragments that have the provided attribute
...
//
//void ShaderTranslationDebugHelper::ValidateComposition(LightningShaderGenerator& generator, LightningFragmentInfo& info, FragmentType::Enum fragmentType)
//{
...
//        LightningShaderIRType* fragmentShaderType = generator.GetCurrentInternalProjectLibrary()->FindType(fragmentName);
...
void ShaderTranslationDebugHelper::OnRunTranslation(Event* e)
{
  ...
  // Grab the lightning, shader, and disassembler results (put them in separate lists to control the order.
  ...
  for(size_t i = 0; i < FragmentType::Size; ++i)
  {
    ...
    lightningResultEntries.PushBack(ShaderTranslationEntry(Lexer::Lightning, BuildString("Lightning", stageName), stageDesc.mShaderCode));
  ...
  // Re-order the entries so it's all lightning, then all shader, then all disassembly
...
LightningShaderIRLibraryRef ShaderTranslationDebugHelper::BuildShaderLibrary(LightningShaderGenerator& generator, LightningShaderIRCompositor::ShaderDefinition& shaderDef)
{
  ...
  LightningShaderIRLibraryRef shaderLibrary = mShaderProject.CompileAndTranslate(shaderDependencies, generator.mFrontEndTranslator);
  return shaderLibrary;
}

bool ShaderTranslationDebugHelper::CompilePipeline(LightningShaderIRType* shaderType, ShaderPipelineDescription& pipeline,
