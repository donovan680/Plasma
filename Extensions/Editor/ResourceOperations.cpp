...
Resource* LoadResourceFromNewContentItem(ResourceManager* resourceManager, ContentItem* newContentItem, Resource* resource)
{
  ...
  if(package.Resources.Size() > 0)
  {
    ...
    else
    {
      ...
      //resource = PL::gResources->ResourceIdMap.FindValue(resourceLibrary->Resources.Back().Id, NULL);
...
{
  return FilePath::Combine(GetUserDocumentsDirectory(), "PlasmaEditor", "Trash");
}
