...
void BugReporter::Reset()
{
  ...
  if (PL::gEditor != nullptr && PL::gEditor->mConfig != nullptr)
  {
    if (EditorConfig* editorConfig = PL::gEditor->mConfig->has(EditorConfig))
      mUsername->SetText(editorConfig->PlasmaHubUsername);
  }
...
void BugReporter::OnSend(Event* event)
{
  ...
  {
    if (EditorConfig* editorConfig = PL::gEditor->mConfig->has(EditorConfig))
      editorConfig->PlasmaHubUsername = mUsername->GetText();
  }
...
void BugReportJob::OnWebResponseComplete(WebResponseEvent* event)
{
  ...
  if (response.StartsWith("Success:"))
  {
    ...
    if (taskIdMatches.Empty())
    {
      String message = String::Format(
        "PlasmaHub returned success, but did not include a TaskID. Please visit %s to find your task, or contact a PlasmaHub administrator.",
  ...
  else
  {
    ...
    PL::gEditor->ShowBrowser(Urls::cUserReportIssue, "Bug Report Form");
  }
