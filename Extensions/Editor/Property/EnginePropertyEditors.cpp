...
public:
  ...
  void Refresh()
  {
    ...
    bool revertTypeDisabled = (mInstance.StoredType->IsA(LightningTypeId(GameSession)) ||
                               mInstance.StoredType->IsA(LightningTypeId(Space)));
...
void RegisterEngineEditors()
{
  ...
  LightningTypeId(ChildRenderGroupList)->Add(new MetaPropertyEditor(&CreateProperty< ResourceListEditor<ChildRenderGroupList> >));
