...
{
  if(boundType == LightningTypeId(float)) ComputeLimits<float>(min, max);
  else if(boundType == LightningTypeId(double)) ComputeLimits<double>(min, max);
  else if(boundType == LightningTypeId(int)) ComputeLimits<int>(min, max);
}

double ConvertValue(AnyParam startValue)
{
  BoundType* boundType = Type::GetBoundType(startValue.StoredType);
  if(boundType == LightningTypeId(float)) return (double)startValue.Get<float>();
  else if(boundType == LightningTypeId(double)) return (double)startValue.Get<double>();
  else if(boundType == LightningTypeId(int)) return (double)startValue.Get<int>();
  return 0.0;
}

Any ConvertValue(BoundType* boundType, double newValue)
{
  if(boundType == LightningTypeId(float)) return float(newValue);
  else if(boundType == LightningTypeId(double)) return double(newValue);
  else if(boundType == LightningTypeId(int)) return int(newValue);
  return 0;
}
