...
LightningDefineType(ColorScheme, builder, type)
{
  ...
  // until we have a lightning lexer setup that colors the class/function names these fields should not be exposed as a property since they currently have no effect
...
void SerializeRGBColor(Serializer& stream, cstr fieldName, uint& colorValue)
{
  if(stream.GetMode() == SerializerMode::Loading)
  {
    ...
    if(serialized)
    {
      u64 fullInt = (u64)ReadHexString(stringRange).mValue;
...
void ColorScheme::Save()
{
  ...
  if(mFilePath.Empty())
    mFilePath = FilePath::Combine(GetUserDocumentsDirectory(), "PlasmaEditor", "ColorSchemes");
  ...
  String userSchemeDirectory = FilePath::Combine(GetUserDocumentsDirectory(), "PlasmaEditor", "ColorSchemes");
...
void ColorScheme::LoadSchemes()
{
  ...
  String userColorSchemeDirectory = FilePath::Combine(GetUserDocumentsDirectory(), "PlasmaEditor", "ColorSchemes");
