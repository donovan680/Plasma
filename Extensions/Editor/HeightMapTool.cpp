...
LightningDefineType(HeightMapTool, builder, type)
{
  ...
  {
    LightningBindFieldProperty(mShowPatchIndex);
    LightningBindFieldProperty(mShowCellIndex);
    LightningBindFieldProperty(mCellIndexType);
  }
...
{
  // Disabled creation
  HeightMap* heightMap = static_cast<HeightMap*>(Tool::GetOrCreateEditComponent(LightningTypeId(HeightMap), cHeightMapName, cHeightMapArchetype, mLastEdited, false));
