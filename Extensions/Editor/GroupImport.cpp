...
// When importing Plasma Engine resource files we strip the resource extension from the filename
// to get the original resource name from the project it was imported from
// if we do not do this then the file PlasmaEngineResource.ResourceType.data
// becomes    PlasmaEngineResourceResourceType.ResourceType.data
// instead of PlasmaEngineResource.ResourceType.data
String StripResourceExtension(StringParam filename)
{
  ...
  // If it contains more than 2 periods this is not one of Plasma Engine's data files
  // as at most a file would be named PlasmaEngineResource.ResourceType.data
  ...
  if (resourceExtensionStart.Begin() != resourceExtensionEnd.Begin())
  {
    ...
    if (type->IsA(LightningTypeId(Resource)))
    {
      ...
      // Plasma Engine created resources files from another project and get the same name
...
void RunGroupImport(ImportOptions& options)
{
  ...
  Status status;
  PL::gContentSystem->BuildContentItems(status, contentToBuild, package);
