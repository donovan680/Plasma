...
namespace Plasma
{
	...
	DocumentEditor* EditorMain::OpenDocumentResource(DocumentResource* docResource)
	{
		...
		else
		{
			...
			ResourceDocument* document = (ResourceDocument*)docManager->Documents.FindValue((u64)docResource->mResourceId.mValue, nullptr);
			if (document == nullptr)
  ...
  Connect(commands->GetCommand(commandName), Events::CommandExecute, editorMain, &EditorMain::memberFunction);
	...
	void CreateEditor(Cog* config, StringParam fileToOpen, StringParam newProjectName)
	{
		...
		//----------------------------------------------------------------------------
		{
			...
			LibraryView* libraryView = editorMain->CreateLibraryView(true, false);
			libraryView->SetName("Library");
			libraryView->SetHideOnClose(true);
			editorMain->mBottomDock = editorMain->AddManagedWidget(libraryView, DockArea::Bottom, true);
			editorMain->mLibrary = libraryView;
		}
...

		{
			...
			propertyViewArea->SetName("Properties");
			propertyViewArea->SetHideOnClose(true);
			propertyViewArea->SetSize(Pixels(dockWidth, 280));
			editorMain->AddManagedWidget(propertyViewArea, DockArea::Right, true);
		...
		//----------------------------------------------------------------------------
		{
			...
			editorMain->mBottomDock->AttachAsTab(console, false);
		...
		editorMain->mLibrary->SwitchToTileView();

		ZPrint("Welcome to the Plasma Editor.\n");
