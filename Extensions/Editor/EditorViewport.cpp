...
{
  uint index = PL::gEditor->mEditInGameViewports.FindIndex(this);
...
void EditorViewport::OnSpaceLevelLoaded(Event* event)
{
  ...
  {
    levelSettings = PL::gFactory->CreateCheckedType(LightningTypeId(Cog), editSpace, CoreArchetypes::LevelSettings, 0, editSpace->GetGameSession());
...
void EditorViewport::OnKeyDown(KeyboardEvent* e)
{
  ...
  else
  {
    //if(e->Key == Keys::F12)
    //{
    //  String fileName = BuildString("PlasmaScreenShot-", GetTimeAndDateStamp(), ".png");
