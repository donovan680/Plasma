...
void ShaderIntrinsicsStaticLightningLibrary::Parse(LightningSpirVFrontEnd* translator)
{
  ...
  LightningShaderIRType* samplerType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::Sampler, lightningSamplerType->Name, lightningSamplerType, spv::StorageClassUniformConstant);
  ...
  CreateImageAndSampler(translator, shaderLibrary, core.RealType, lightningDepthImage2d, lightningSampledDepthImage2d, spv::Dim2D, 1);
  CreateImageAndSampler(translator, shaderLibrary, core.RealType, lightningImageCube, lightningSampledImageCube, spv::DimCube, 0);
  CreateStorageImage(translator, shaderLibrary, core.RealType, lightningStorageImage2d, spv::Dim2D, 0, spv::ImageFormatRgba32f);
...
void ShaderIntrinsicsStaticLightningLibrary::CreateImageAndSampler(LightningSpirVFrontEnd* translator, LightningShaderIRLibrary* shaderLibrary,
  Lightning::BoundType* lightningSampledType, Lightning::BoundType* lightningImageType, Lightning::BoundType* lightningSampledImageType,
  int dimension, int depthMode)
{
  ...
  LightningShaderIRType* imageType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::Image, lightningImageType->Name, lightningImageType, spv::StorageClassUniformConstant);
  ...
  LightningShaderIRType* sampledImageType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::SampledImage, lightningSampledImageType->Name, lightningSampledImageType, spv::StorageClassUniformConstant);
  sampledImageType->mParameters.PushBack(imageType);
  translator->MakeShaderTypeMeta(sampledImageType, nullptr);
}

void ShaderIntrinsicsStaticLightningLibrary::CreateStorageImage(LightningSpirVFrontEnd* translator, LightningShaderIRLibrary* shaderLibrary,
  Lightning::BoundType* lightningSampledType, Lightning::BoundType* lightningImageType,
...
void ShaderIntrinsicsStaticLightningLibrary::PopulateStageRequirementsData(LightningShaderIRLibrary* shaderLibrary)
{
  ...
  for(size_t i = 0; i < lightningLibrary->OwnedFunctions.Size(); ++i)
  {
    ...
    if(lightningFunction->HasAttribute(SpirVNameSettings::mRequiresPixelAttribute))
      shaderLibrary->mStageRequirementsData[lightningFunction].Combine(nullptr, lightningFunction->Location, ShaderStage::Pixel);
  }
}
