...
LightningShaderIRType* LightningSpirVFrontEnd::MakeTypeInternal(LightningShaderIRLibrary* shaderLibrary, ShaderIRTypeBaseType::Enum baseType, StringParam typeName, Lightning::BoundType* lightningType, spv::StorageClass storageClass)
{
...
LightningShaderIRType* LightningSpirVFrontEnd::MakeTypeAndPointer(LightningShaderIRLibrary* shaderLibrary, ShaderIRTypeBaseType::Enum baseType, StringParam typeName, Lightning::BoundType* lightningType, spv::StorageClass pointerStorageClass)
{
  ...
  LightningShaderIRType* shaderType = MakeTypeInternal(shaderLibrary, baseType, typeName, lightningType, spv::StorageClassGeneric);
  LightningShaderIRType* pointerType = MakeTypeInternal(shaderLibrary, ShaderIRTypeBaseType::Pointer, BuildString(typeName, "_ptr"), lightningType, pointerStorageClass);
...
LightningShaderIRType* LightningSpirVFrontEnd::MakeCoreType(LightningShaderIRLibrary* shaderLibrary, ShaderIRTypeBaseType::Enum baseType, size_t components, LightningShaderIRType* componentType, Lightning::BoundType* lightningType, bool makePointerType)
{
  LightningShaderIRType* shaderType = MakeTypeAndPointer(shaderLibrary, baseType, lightningType->Name, lightningType, spv::StorageClassFunction);
  shaderType->mComponentType = componentType;
  shaderType->mComponents = components;
  return shaderType;
}

LightningShaderIRType* LightningSpirVFrontEnd::MakeStructType(LightningShaderIRLibrary* shaderLibrary, StringParam typeName, Lightning::BoundType* lightningType, spv::StorageClass pointerStorageClass)
{
  return MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::Struct, typeName, lightningType, pointerStorageClass);
}

LightningShaderIRType* LightningSpirVFrontEnd::FindOrCreateInterfaceType(LightningShaderIRLibrary* shaderLibrary, StringParam baseTypeName, Lightning::BoundType* lightningType, ShaderIRTypeBaseType::Enum baseType, spv::StorageClass storageClass)
{
...
LightningShaderIRType* LightningSpirVFrontEnd::FindOrCreateInterfaceType(LightningShaderIRLibrary* shaderLibrary, Lightning::BoundType* lightningType, ShaderIRTypeBaseType::Enum baseType, spv::StorageClass storageClass)
{
  return FindOrCreateInterfaceType(shaderLibrary, lightningType->Name, lightningType, baseType, storageClass);
}

LightningShaderIRType* LightningSpirVFrontEnd::FindOrCreatePointerInterfaceType(LightningShaderIRLibrary* shaderLibrary, LightningShaderIRType* valueType, spv::StorageClass storageClass)
{
  ErrorIf(valueType->IsPointerType(), "Type must be a value type");
  LightningShaderIRType* pointerType = FindOrCreateInterfaceType(shaderLibrary, valueType->mName, valueType->mLightningType, ShaderIRTypeBaseType::Pointer, storageClass);

  pointerType->mDereferenceType = valueType;
  return pointerType;
}

ShaderIRTypeMeta* LightningSpirVFrontEnd::MakeShaderTypeMeta(LightningShaderIRType* shaderType, Lightning::NodeList<Lightning::AttributeNode>* nodeAttributeList)
{
  ...
  // Parse attributes if possible (interface types don't have backing lightning types)
...
bool LightningSpirVFrontEnd::Translate(Lightning::SyntaxTree& syntaxTree, LightningShaderIRProject* project, LightningShaderIRLibrary* library)
{
...
void LightningSpirVFrontEnd::ParseAttributes(Lightning::Array<Lightning::Attribute>& lightningAttributes, Lightning::NodeList<Lightning::AttributeNode>* attributeNodes, ShaderIRTypeMeta* typeMeta)
{
  ...
  for(size_t i = 0; i < shaderAttributes.Size(); ++i)
  {
    ...
    {
      fragmentTypeAttributeIndices.PushBack(i);
      ValidateSingleParamAttribute(shaderAttribute, nameSettings.mMaxVerticesParam, Lightning::ConstantType::Integer, false);
    }
...
void LightningSpirVFrontEnd::ParseAttributes(Lightning::Array<Lightning::Attribute>& lightningAttributes, Lightning::NodeList<Lightning::AttributeNode>* attributeNodes, ShaderIRFunctionMeta* functionMeta)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void LightningSpirVFrontEnd::ParseAttributes(Lightning::Array<Lightning::Attribute>& lightningAttributes, Lightning::NodeList<Lightning::AttributeNode>* attributeNodes, ShaderIRFieldMeta* fieldMeta)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  for(size_t i = 0; i < shaderAttributes.Size(); ++i)
  {
    ...
    {
      LightningShaderIRType* fieldShaderType = FindType(fieldMeta->mLightningType, nullptr);
...
void LightningSpirVFrontEnd::ParseLightningAttributes(Lightning::Array<Lightning::Attribute>& lightningAttributes, Lightning::NodeList<Lightning::AttributeNode>* attributeNodes, ShaderIRAttributeList& shaderAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void LightningSpirVFrontEnd::ParseLightningAttribute(Lightning::Attribute& lightningAttribute, Lightning::AttributeNode* attributeNode, ShaderIRAttributeList& shaderAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void LightningSpirVFrontEnd::ValidateAllowedAttributes(ShaderIRAttributeList& shaderAttributes, HashMap<String, AttributeInfo>& allowedAttributes, StringParam errorTypeName)
{
  for(size_t i = 0; i < shaderAttributes.Size(); ++i)
  {
...
void LightningSpirVFrontEnd::ValidateNameOverrideAttribute(ShaderIRAttribute* shaderAttribute)
{
  ...
  if(paramCount == 1)
  {
    ...
    {
      String msg = String::Format("Parameter '%s' must be a valid lightning uppercase identifier.", SpirVNameSettings::mNameOverrideParam.c_str());
      SendTranslationError(shaderAttribute->GetLocation(), msg);
      return;
    }
  }
...
void LightningSpirVFrontEnd::ValidateSingleParamAttribute(ShaderIRAttribute* shaderAttribute, StringParam expectedParamName, Lightning::ConstantType::Enum expectedParamType, bool allowEmptyName)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  if(paramCount == 0)
  {
    String msg = String::Format("Not enough parameters to attribute '%s'. Signature must be '%s : %s'",
      shaderAttribute->mAttributeName.c_str(), expectedParamName.c_str(), Lightning::ConstantType::Names[expectedParamType]);
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
  ...
  if(actualParamType != expectedParamType)
  {
    String msg = String::Format("Invalid parameter type '%s' to attribute '%s'. Signature must be '%s : %s'",
      Lightning::ConstantType::Names[actualParamType], shaderAttribute->mAttributeName.c_str(), expectedParamName.c_str(), Lightning::ConstantType::Names[expectedParamType]);
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
  ...
  if((actualParamName.Empty() && !allowEmptyName) || (actualParamName != expectedParamName))
  {
    String msg = String::Format("Invalid parameter name '%s' to attribute '%s'. Signature must be '%s : %s'",
      actualParamName.c_str(), shaderAttribute->mAttributeName.c_str(), expectedParamName.c_str(), Lightning::ConstantType::Names[expectedParamType]);
    SendTranslationError(shaderAttribute->GetLocation(), msg);
    return;
  }
}
...
void LightningSpirVFrontEnd::ValidateAttributeDependencies(ShaderIRAttribute* shaderAttribute, ShaderIRAttributeList& shaderAttributeList, Array<String>& dependencies)
{
...
void LightningSpirVFrontEnd::ValidateAttributeExclusions(ShaderIRAttribute* shaderAttribute, ShaderIRAttributeList& shaderAttributeList, Array<String>& exclusions)
{
...
void LightningSpirVFrontEnd::ValidateHardwareBuiltIn(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* shaderAttribute, bool isInput)
{
  // If this is an explicit attribute then check to see if it matches something in the uniform buffers
  if(!shaderAttribute->mImplicitAttribute)
  {
...
void LightningSpirVFrontEnd::ValidateAndParseComputeAttributeParameters(ShaderIRAttribute* shaderAttribute, ShaderIRTypeMeta* typeMeta)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
String LightningSpirVFrontEnd::BuildFunctionTypeString(Lightning::BoundType* lightningReturnType, Array<Lightning::Type*>& signature, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::GenerateFunctionType(Lightning::SyntaxNode* locationNode, LightningShaderIRFunction* function, Lightning::Function* lightningFunction, LightningSpirVFrontEndContext* context)
{
  ...
  if(lightningFunctionType != nullptr)
  {
    ...
    for(size_t i = 0; i < lightningFunctionType->Parameters.Size(); ++i)
    {
      ...
      // Make sure to pass through the correct shader type (pointer/value) based upon the lightning type.
...
void LightningSpirVFrontEnd::GenerateFunctionType(Lightning::SyntaxNode* locationNode, LightningShaderIRFunction* function, Lightning::BoundType* lightningReturnType, Array<Lightning::Type*>& signature, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIRFunction* LightningSpirVFrontEnd::GenerateIRFunction(Lightning::SyntaxNode* node, Lightning::NodeList<Lightning::AttributeNode>* nodeAttributeList, LightningShaderIRType* owningType, Lightning::Function* lightningFunction, StringParam functionName, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::AddImplements(Lightning::SyntaxNode* node, Lightning::Function* lightningFunction, LightningShaderIRFunction* shaderFunction, StringParam functionName, LightningSpirVFrontEndContext* context)
{
  if(lightningFunction == nullptr)
    return;
  ...
  // Find the function from lightning's bound type (check static or instance as appropriate)
  ...
  // We found a replacement function. Add a duplicate mapping for the lightning function we're replacing to the target shader function
...
void LightningSpirVFrontEnd::AddRuntimeArray(Lightning::MemberVariableNode* node, LightningShaderIRType* varType, ShaderIRFieldMeta* fieldMeta, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::AddGlobalVariable(Lightning::MemberVariableNode* node, LightningShaderIRType* varType, ShaderIRFieldMeta* fieldMeta, spv::StorageClass storageClass, LightningSpirVFrontEndContext* context)
{
...
GlobalVariableData* LightningSpirVFrontEnd::CreateGlobalVariable(spv::StorageClass storageClass, LightningShaderIRType* varType, StringParam varName, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIRType* pointerType = varType->mPointerType;
  ...
  LightningShaderIROp* globalVar = BuildIROpNoBlockAdd(OpType::OpVariable, pointerType, context);
...
void LightningSpirVFrontEnd::GeneratePreConstructor(Lightning::ClassNode*& node, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIRFunction* function = GenerateIRFunction(node, nullptr, context->mCurrentType, lightningFunction, lightningFunction->Name, context);

  BasicBlock* currentBlock = BuildBlock(String(), context);
  context->mCurrentBlock = currentBlock;

  // Declare the self param
  LightningShaderIROp* selfOp = BuildIROp(&function->mParameterBlock, OpType::OpFunctionParameter, currentType->mPointerType, context);
  ...
  for(size_t i = 0; i < node->Variables.Size(); ++i)
  {
    ...
    LightningShaderIROp* memberPtrOp = BuildIROp(currentBlock, OpType::OpAccessChain, memberType->mPointerType, selfOp, offsetConstant, context);
...
void LightningSpirVFrontEnd::GenerateDefaultConstructor(Lightning::ClassNode*& node, LightningSpirVFrontEndContext* context)
{
  ...
  // unfortunately don't have a backing lightning function so manually generate the signature.
...
void LightningSpirVFrontEnd::GenerateStaticVariableInitializer(Lightning::MemberVariableNode*& node, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::DefaultConstructType(Lightning::SyntaxNode* locationNode, LightningShaderIRType* type, LightningShaderIROp* selfVar, LightningSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void LightningSpirVFrontEnd::GenerateFunctionParameters(Lightning::GenericFunctionNode* node, LightningSpirVFrontEndContext* context)
{
  LightningShaderIRType* currentType = context->mCurrentType;
  // Debug sanity
  context->mCurrentBlock = nullptr;
  ...
  LightningShaderIRFunction* function = mLibrary->FindFunction(lightningFunction, nullptr);
  ...
  for(size_t i = 0; i < node->Parameters.Size(); ++i)
  {
    ...
    LightningShaderIROp* op = BuildIROp(&function->mParameterBlock, OpType::OpFunctionParameter, shaderParameterType, context);
...
void LightningSpirVFrontEnd::GenerateEntryPoint(Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::WalkConstructorCallNode(Lightning::FunctionCallNode*& node, Lightning::StaticTypeNode* constructorNode, LightningSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
  ...
  // Deal with auto-generated default constructors (they don't have a generated lightning function)
...
void LightningSpirVFrontEnd::WalkMemberAccessCallNode(Lightning::FunctionCallNode*& node, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void LightningSpirVFrontEnd::WalkMemberAccessFunctionCallNode(Lightning::FunctionCallNode*& node, Lightning::MemberAccessNode* memberAccessNode, LightningShaderIRFunction* shaderFunction, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::WalkMemberAccessExtensionInstructionCallNode(Lightning::FunctionCallNode*& node, Lightning::MemberAccessNode* memberAccessNode, SpirVExtensionInstruction* extensionInstruction, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::WalkStaticTypeOrCreationCallNode(Lightning::StaticTypeNode*& node, LightningSpirVFrontEndContext* context)
{
  SendTranslationError(node->Location, "StaticTypeOrCreationCallNode not translatable.");
  context->PushIRStack(GenerateDummyIR(node, context));
}

void LightningSpirVFrontEnd::WalkExpressionInitializerNode(Lightning::ExpressionInitializerNode*& node, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::WalkUnaryOperationNode(Lightning::UnaryOperatorNode*& node, LightningSpirVFrontEndContext* context)
{
  ...
  if(node->Operator->TokenId == Lightning::Grammar::Dereference)
  {
    ...
    LightningShaderIROp* dereferenceOp = BuildCurrentBlockIROp(OpType::OpLoad, operand->mResultType->mDereferenceType, operand, context);
  ...
  if(node->AccessedMember)
  {
    ...
    if(node->AccessedGetterSetter != nullptr)
    {
      ...
      if(node->IoUsage == Lightning::IoMode::ReadRValue)
      {
        ...
        // If this is an existing lightning function then we have to translate this member access into a function call
        if(shaderFunction != nullptr)
        {
          Array<ILightningShaderIR*> arguments;
    ...
    MemberAccessResolverIRFn fieldResolver = mLibrary->FindFieldResolver(leftOperandType->mLightningType, node->AccessedField);
    ...
    if(operandResultOp->IsResultPointerType())
    {
      ...
      LightningShaderIROp* memberAccessOp = BuildCurrentBlockAccessChain(memberType, operandResultOp, memberIndexConstant, context);
    ...
    else
    {
      ...
      LightningShaderIROp* memberAccessOp = BuildCurrentBlockIROp(OpType::OpCompositeExtract, memberType, operandResultOp, memberIndexLiteral, context);
      context->PushIRStack(memberAccessOp);
    }
  }
  else
...
{
  Lightning::SyntaxNode* initializer;
...
void LightningSpirVFrontEnd::WalkGenericLoop(Lightning::SyntaxNode* initializerNode, Lightning::SyntaxNode* iterator, Lightning::ConditionalLoopNode* conditionalNode, Lightning::LoopScopeNode* loopScopeNode, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::GenerateLoopHeaderBlock(BasicBlock* headerBlock, BasicBlock* branchTarget, BasicBlock* mergeBlock, BasicBlock* continueBlock, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::GenerateLoopConditionBlock(Lightning:: ConditionalLoopNode* conditionalNode, BasicBlock* conditionBlock, BasicBlock* branchTrueBlock, BasicBlock* branchFalseBlock, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::GenerateLoopStatements(Lightning::LoopScopeNode* loopScopeNode, BasicBlock* loopBlock, BasicBlock* mergeBlock, BasicBlock* continueBlock, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::GenerateLoopContinueBlock(Lightning::SyntaxNode* iterator, BasicBlock* continueBlock, BasicBlock* headerBlock, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::FixBlockTerminators(BasicBlock* block, LightningSpirVFrontEndContext* context)
{
  ...
  if(firstTerminatorIndex >= opCount)
  {
    ...
    // These statements should be unreachable otherwise we'd have a lightning error.
...
bool LightningSpirVFrontEnd::ResolveSetter(Lightning::BinaryOperatorNode* node, LightningShaderIROp* resultValue, Lightning::SyntaxNode* resultNode, LightningSpirVFrontEndContext* context)
{
  Lightning::MemberAccessNode* memberAccessNode = Lightning::Type::DynamicCast<Lightning::MemberAccessNode*>(node->LeftOperand);
...
ILightningShaderIR* LightningSpirVFrontEnd::PerformBinaryOp(Lightning::BinaryOperatorNode*& node, OpType opType, LightningSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
ILightningShaderIR* LightningSpirVFrontEnd::PerformBinaryOp(Lightning::BinaryOperatorNode*& node, OpType opType, ILightningShaderIR* lhs, ILightningShaderIR* rhs, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::PerformBinaryAssignmentOp(Lightning::BinaryOperatorNode*& node, OpType opType, LightningSpirVFrontEndContext* context)
{
  // Walk both sides of the op
  ILightningShaderIR* leftInstruction = WalkAndGetResult(node->LeftOperand, context);
  ILightningShaderIR* rightInstruction = WalkAndGetResult(node->RightOperand, context);
...
void LightningSpirVFrontEnd::PerformBinaryAssignmentOp(Lightning::BinaryOperatorNode*& node, OpType opType, ILightningShaderIR* lhs, ILightningShaderIR* rhs, LightningSpirVFrontEndContext* context)
{
...
ILightningShaderIR* LightningSpirVFrontEnd::PerformUnaryOp(Lightning::UnaryOperatorNode*& node, OpType opType, LightningSpirVFrontEndContext* context)
{
  LightningShaderIRType* resultType = FindType(node->ResultType, node);
...
ILightningShaderIR* LightningSpirVFrontEnd::PerformUnaryIncDecOp(Lightning::UnaryOperatorNode*& node, ILightningShaderIR* constantOne, OpType opType, LightningSpirVFrontEndContext* context)
{
...
ILightningShaderIR* LightningSpirVFrontEnd::PerformTypeCast(Lightning::TypeCastNode*& node, OpType opType, LightningSpirVFrontEndContext* context)
{
  LightningShaderIRType* resultType = FindType(node->ResultType, node);
...
LightningShaderIROp* LightningSpirVFrontEnd::GetConstant(LightningShaderIRType* type, StringParam value, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::GetConstant(LightningShaderIRType* type, Lightning::Any value, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIROp* opConstant = mLibrary->FindConstantOp(constantKey, nullptr);
...
LightningShaderIROp* LightningSpirVFrontEnd::ConstructCompositeFromScalar(BasicBlock* block, LightningShaderIRType* compositeType, ILightningShaderIR* scalar, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::AddSpecializationConstant(Lightning::MemberVariableNode* node, LightningShaderIRType* varType, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::AddSpecializationConstantRecursively(void* key, LightningShaderIRType* varType, StringParam varName, LightningShaderIRConstantLiteral* literalValue, Lightning::CodeLocation& codeLocation, LightningSpirVFrontEndContext* context)
{
  ...
  {
    LightningShaderIROp* specConstantCompositeOp = CreateSpecializationConstant(key, OpType::OpSpecConstantComposite, varType, context);
    ...
    for(size_t i = 0; i < varType->mComponents; ++i)
    {
      ...
      LightningShaderIROp* constituent = AddSpecializationConstantRecursively(nullptr, componentType, fullSubVarName, nullptr, codeLocation, context);
  ...
  if(varType->mBaseType == ShaderIRTypeBaseType::Struct)
  {
    ...
    for(size_t i = 0; i < varType->mParameters.Size(); ++i)
    {
      ...
      LightningShaderIROp* constituent = AddSpecializationConstantRecursively(nullptr, subType, fullSubVarName, nullptr, codeLocation, context);
      specConstantCompositeOp->mArguments.PushBack(constituent);
    }
    return specConstantCompositeOp;
  }
...
LightningShaderIROp* LightningSpirVFrontEnd::CreateSpecializationConstant(void* key, OpType opType, LightningShaderIRType* varType, LightningSpirVFrontEndContext* context)
{
  ...
  // This is used to find the constant at a later point in time via the key (such as the lightning field).
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildIROpNoBlockAdd(OpType opType, LightningShaderIRType* resultType, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, LightningShaderIRType* resultType, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, LightningShaderIRType* resultType, ILightningShaderIR* arg0, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, LightningShaderIRType* resultType, ILightningShaderIR* arg0, ILightningShaderIR* arg1, LightningSpirVFrontEndContext* context)
{
  LightningShaderIROp* result = BuildIROp(block, opType, resultType, context);
  result->mArguments.PushBack(arg0);
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildIROp(BasicBlock* block, OpType opType, LightningShaderIRType* resultType, ILightningShaderIR* arg0, ILightningShaderIR* arg1, ILightningShaderIR* arg2, LightningSpirVFrontEndContext* context)
{
  LightningShaderIROp* result = BuildIROp(block, opType, resultType, context);
  result->mArguments.PushBack(arg0);
  result->mArguments.PushBack(arg1);
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, LightningShaderIRType* resultType, LightningSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, LightningShaderIRType* resultType, ILightningShaderIR* arg0, LightningSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, arg0, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, LightningShaderIRType* resultType, ILightningShaderIR* arg0, ILightningShaderIR* arg1, LightningSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, arg0, arg1, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildCurrentBlockIROp(OpType opType, LightningShaderIRType* resultType, ILightningShaderIR* arg0, ILightningShaderIR* arg1, ILightningShaderIR* arg2, LightningSpirVFrontEndContext* context)
{
  return BuildIROp(context->GetCurrentBlock(), opType, resultType, arg0, arg1, arg2, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildCurrentBlockAccessChain(LightningShaderIRType* baseResultType, LightningShaderIROp* selfInstance, ILightningShaderIR* arg0, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIROp* accessChainOp = BuildCurrentBlockIROp(OpType::OpAccessChain, resultPointerType, selfInstance, context);
  accessChainOp->mArguments.PushBack(arg0);
  return accessChainOp;
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildCurrentBlockAccessChain(LightningShaderIRType* baseResultType, LightningShaderIROp* selfInstance, ILightningShaderIR* arg0, ILightningShaderIR* arg1, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildDecorationOp(BasicBlock* block, ILightningShaderIR* decorationTarget, spv::Decoration decorationType, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildDecorationOp(BasicBlock* block, ILightningShaderIR* decorationTarget, spv::Decoration decorationType, int decorationValue, LightningSpirVFrontEndContext* context)
{
  LightningShaderIRConstantLiteral* decorationTypeLiteral = GetOrCreateConstantIntegerLiteral(decorationType);
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildMemberDecorationOp(BasicBlock* block, ILightningShaderIR* decorationTarget, int memberOffset, spv::Decoration decorationType, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildMemberDecorationOp(BasicBlock* block, ILightningShaderIR* decorationTarget, int memberOffset, spv::Decoration decorationType, int decorationValue, LightningSpirVFrontEndContext* context)
{
  LightningShaderIROp* resultOp = BuildIROp(block, OpType::OpMemberDecorate, nullptr, decorationTarget, context);
  resultOp->mArguments.PushBack(GetOrCreateConstantIntegerLiteral(memberOffset));
  resultOp->mArguments.PushBack(GetOrCreateConstantIntegerLiteral(decorationType));
...
LightningShaderIROp* LightningSpirVFrontEnd::BuildOpVariable(BasicBlock* block, LightningShaderIRType* resultType, int storageConstant, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::WalkAndGetValueTypeResult(BasicBlock* block, Lightning::SyntaxNode* node, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::WalkAndGetValueTypeResult(Lightning::SyntaxNode* node, LightningSpirVFrontEndContext* context)
{
  ILightningShaderIR* nodeResult = WalkAndGetResult(node, context);
...
LightningShaderIROp* LightningSpirVFrontEnd::GetOrGenerateValueTypeFromIR(BasicBlock* block, ILightningShaderIR* instruction, LightningSpirVFrontEndContext* context)
{
...
LightningShaderIROp* LightningSpirVFrontEnd::GetOrGenerateValueTypeFromIR(ILightningShaderIR* instruction, LightningSpirVFrontEndContext* context)
{
  return GetOrGenerateValueTypeFromIR(context->GetCurrentBlock(), instruction, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::GetOrGeneratePointerTypeFromIR(BasicBlock* block, ILightningShaderIR* instruction, LightningSpirVFrontEndContext* context)
{
  LightningShaderIROp* op = instruction->As<LightningShaderIROp>();
  if(op->IsResultPointerType())
    return op;
...
LightningShaderIROp* LightningSpirVFrontEnd::GetOrGeneratePointerTypeFromIR(ILightningShaderIR* instruction, LightningSpirVFrontEndContext* context)
{
  return GetOrGeneratePointerTypeFromIR(context->GetCurrentBlock(), instruction, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildStoreOp(ILightningShaderIR* target, ILightningShaderIR* source, LightningSpirVFrontEndContext* context, bool forceLoadStore)
{
  return BuildStoreOp(context->GetCurrentBlock(), target, source, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::BuildStoreOp(BasicBlock* block, ILightningShaderIR* target, ILightningShaderIR* source, LightningSpirVFrontEndContext* context, bool forceLoadStore)
{
  ...
  if(sourceOp->IsResultPointerType())
  {
    ...
    if(forceLoadStore)
    {
      LightningShaderIROp* loadOp = BuildIROp(block, OpType::OpLoad, sourceOp->mResultType->mDereferenceType, sourceOp, context);
...
void LightningSpirVFrontEnd::GetFunctionCallArguments(Lightning::FunctionCallNode* node, Lightning::MemberAccessNode* memberAccessNode, Array<ILightningShaderIR*>& arguments, LightningSpirVFrontEndContext* context)
{
  ILightningShaderIR* thisOp = nullptr;
...
void LightningSpirVFrontEnd::GetFunctionCallArguments(Lightning::FunctionCallNode* node, ILightningShaderIR* thisOp, Array<ILightningShaderIR*>& arguments, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::WriteFunctionCallArguments(Array<ILightningShaderIR*> arguments, LightningShaderIRType* functionType, LightningShaderIROp* functionCallOp, LightningSpirVFrontEndContext* context)
{
...
void LightningSpirVFrontEnd::WriteFunctionCallArgument(ILightningShaderIR* argument, LightningShaderIROp* functionCallOp, LightningShaderIRType* paramType, LightningSpirVFrontEndContext* context)
{
  ...
  else if(paramType->IsPointerType() && paramOp->mResultType->mStorageClass != paramType->mStorageClass)
  {
    ...
    LightningShaderIRType* localVarPointerType = FindOrCreatePointerInterfaceType(mLibrary, paramType->mDereferenceType, paramType->mStorageClass);
    LightningShaderIROp* localVarOp = BuildOpVariable(localVarPointerType, context);
    ...
    context->mFunctionPostambleCopies.PushBack(LightningSpirVFrontEndContext::PostableCopy(paramOp, localVarOp));
...
LightningShaderIROp* LightningSpirVFrontEnd::GenerateFunctionCall(LightningShaderIRFunction* shaderFunction, LightningSpirVFrontEndContext* context)
{
  BasicBlock* currentBlock = context->GetCurrentBlock();
...
void LightningSpirVFrontEnd::WriteFunctionCallPostamble(LightningSpirVFrontEndContext* context)
{
  ...
  {
    LightningSpirVFrontEndContext::PostableCopy& copy = context->mFunctionPostambleCopies[i];
    BuildStoreOp(copy.mDestination, copy.mSource, context);
  }
  context->mFunctionPostambleCopies.Clear();
}

void LightningSpirVFrontEnd::WriteFunctionCall(Array<ILightningShaderIR*> arguments, LightningShaderIRFunction* shaderFunction, LightningSpirVFrontEndContext* context)
{
  // Generate the function call but don't add it to the block yet (so we can collect all arguments first)
...
LightningShaderIROp* LightningSpirVFrontEnd::GenerateBoolToIntegerCast(BasicBlock* block, LightningShaderIROp* source, LightningShaderIRType* destType, LightningSpirVFrontEndContext* context)
{
  LightningShaderIROp* zero = GetIntegerConstant(0, context);
  LightningShaderIROp* one = GetIntegerConstant(1, context);
  return GenerateFromBoolCast(block, source, destType, zero, one, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::GenerateFromBoolCast(BasicBlock* block, LightningShaderIROp* source, LightningShaderIRType* destType, ILightningShaderIR* zero, ILightningShaderIR* one, LightningSpirVFrontEndContext* context)
{
  ...
  ILightningShaderIR* falseValue = zero;
  
  // Handle vector types
  if(destType->mComponents != 1)
  {
    // Construct the result vector types from the individual zero/one scalars
    LightningShaderIROp* zeroVec = BuildIROp(block, OpType::OpCompositeConstruct, destType, context);
...
LightningShaderIROp* LightningSpirVFrontEnd::GenerateIntegerToBoolCast(BasicBlock* block, LightningShaderIROp* source, LightningShaderIRType* destType, LightningSpirVFrontEndContext* context)
{
  ILightningShaderIR* zero = GetIntegerConstant(0, context);
  return GenerateToBoolCast(block, OpType::OpINotEqual, source, destType, zero, context);
}

LightningShaderIROp* LightningSpirVFrontEnd::GenerateToBoolCast(BasicBlock* block, OpType op, LightningShaderIROp* source, LightningShaderIRType* destType, ILightningShaderIR* zero, LightningSpirVFrontEndContext* context)
{
  ...
  if(sourceType->mComponents != 1)
  {
    ...
    LightningShaderIROp* zeroVec = BuildIROp(block, OpType::OpCompositeConstruct, sourceType, context);
...
bool LightningSpirVFrontEnd::CheckForNonCopyableType(LightningShaderIRType* shaderType, Lightning::ExpressionNode* node, LightningSpirVFrontEndContext* context, bool generateDummyResult)
{
...
bool LightningSpirVFrontEnd::ValidateResultType(ILightningShaderIR* instruction, ShaderIRTypeBaseType::Enum expectedType, Lightning::CodeLocation& codeLocation, bool throwException)
{
  LightningShaderIROp* op = instruction->As<LightningShaderIROp>();
...
void LightningSpirVFrontEnd::SendTranslationError(Lightning::CodeLocation& codeLocation, StringParam shortMsg, StringParam fullMsg)
{
