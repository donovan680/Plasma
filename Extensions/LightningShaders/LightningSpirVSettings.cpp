...
void BuiltInBlockDescription::AddField(Lightning::BoundType* type, StringParam fieldName, spv::BuiltIn builtInId, StringParam attribute)
{
...
void LightningShaderSpirVSettings::SetDefaultUniformBufferDescription(int bindingId, int descriptorSetId, StringParam debugName)
{
  ReturnIf(mFinalized, , "Cannot set built-in names once finalized");
...
bool LightningShaderSpirVSettings::IsValidUniform(FragmentType::Enum fragmentType, StringParam fieldType, StringParam fieldName)
{
...
bool LightningShaderSpirVSettings::IsValidHardwareBuiltIn(FragmentType::Enum fragmentType, StringParam fieldType, StringParam fieldName, bool isInput)
{
  ShaderFieldKey fieldKey(fieldName, fieldType);
...
void LightningShaderSpirVSettings::ValidateBuiltInNames(BuiltInBlockDescription& blockDescription, HashMap<String, spv::BuiltIn>& keyMappings)
{
  ...
  for(size_t i = 0; i < mUniformBufferDescriptions.Size(); ++i)
  {
    ...
    for(size_t j = 0; j < description.mFields.Size(); ++j)
    {
      ...
      ErrorIf(isHardwardBuiltIn, "AppBuiltIn '%s : %s' matches a HardwareBuiltIn. "
        ...
        fieldMeta->mLightningName.c_str(), fieldMeta->mLightningType->ToString().c_str());
    }
  }
}
