...
public:
  ...
  /// Lightning library to shader library map. Needed during recursion to find the shader 
...
/// Helper lightning AST walker to find what symbols have various stage requirements and emit
...
private:
  ...
  void MergeCurrent(LightningShaderIRLibrary* shaderLibrary, Lightning::Member* lightningMember, Lightning::SyntaxNode* node, StageRequirementsGathererContext* context);
  ...
  void CheckAndDispatchErrors(Lightning::Member* lightningObject, Lightning::ReflectionObject* owner, StageRequirementsGathererContext* context);
