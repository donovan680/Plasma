...
void ResolveIntBitCount(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
...
void RegisterBitOps(LightningSpirVFrontEnd* translator, LightningShaderIRLibrary* shaderLibrary, TypeGroups& types)
{
  ...
  for(size_t i = 0; i < types.mIntegerVectorTypes.Size(); ++i)
  {
    ...
    mathTypeResolver.RegisterFunctionResolver(GetStaticFunction(mathType, "CountBits", lightningTypeName), ResolveIntBitCount);

    opResolvers.RegisterBinaryOpResolver(lightningType, lightningType, Lightning::Grammar::BitwiseOr, ResolveBinaryOperator<OpType::OpBitwiseOr>);
    opResolvers.RegisterBinaryOpResolver(lightningType, lightningType, Lightning::Grammar::BitwiseAnd, ResolveBinaryOperator<OpType::OpBitwiseAnd>);
    opResolvers.RegisterBinaryOpResolver(lightningType, lightningType, Lightning::Grammar::BitwiseXor, ResolveBinaryOperator<OpType::OpBitwiseXor>);
