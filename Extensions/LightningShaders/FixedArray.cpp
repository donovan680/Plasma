...
void FixedArrayDefaultConstructor(LightningSpirVFrontEnd* translator, Lightning::Type* resultType, LightningSpirVFrontEndContext* context)
{
...
void FixedArrayBackupConstructor(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* fnCallNode, Lightning::StaticTypeNode* staticTypeNode, LightningSpirVFrontEndContext* context)
{
  FixedArrayDefaultConstructor(translator, staticTypeNode->ResultType, context);
}

bool ValidateIndexLiteral(LightningSpirVFrontEnd* translator, Lightning::ExpressionNode* node, LightningShaderIROp* indexOperand, int maxValue, LightningSpirVFrontEndContext* context)
{
...
void ResolveFixedArrayGet(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  ...
  ILightningShaderIR* accessChainOp = translator->BuildCurrentBlockAccessChain(elementType, selfInstance, indexOperand, context);

  context->PushIRStack(accessChainOp);
}

void ResolveFixedArraySet(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  // Get the 'this' array type and component type
  Lightning::Type* lightningArrayType = memberAccessNode->LeftOperand->ResultType;
  LightningShaderIRType* arrayType = translator->FindType(lightningArrayType, memberAccessNode->LeftOperand);
...
void ResolveFixedArrayCount(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
...
void FixedArrayExpressionInitializerResolver(LightningSpirVFrontEnd* translator, Lightning::ExpressionInitializerNode*& node, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIROp * compositeConstructOp = translator->BuildIROpNoBlockAdd(OpType::OpCompositeConstruct, fixedArrayType, context);
  for(size_t i = 0; i < node->InitializerStatements.Size(); ++i)
  {
    ...
    {
      Lightning::MemberAccessNode* memberAccessNode = Lightning::Type::DynamicCast<Lightning::MemberAccessNode*>(addCallNode->LeftOperand);
  ...
  Lightning::FunctionCallNode* functionCallNode = Lightning::Type::DynamicCast<Lightning::FunctionCallNode*>(localVariableNode->InitialValue);
...
void FixedArrayResolver(LightningSpirVFrontEnd* translator, Lightning::BoundType* lightningFixedArrayType)
{
  ...
  LightningShaderIRType* fixedArrayType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::FixedArray, lightningFixedArrayType->Name, lightningFixedArrayType, spv::StorageClassFunction);
  ...
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningFixedArrayType, Lightning::OperatorGet, intTypeName), ResolveFixedArrayGet);
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningFixedArrayType, Lightning::OperatorSet, intTypeName, lightningElementType->ToString()), ResolveFixedArraySet);
...
void ResolveRuntimeArrayGet(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIROp* selfInstance = translator->GetOrGeneratePointerTypeFromIR(leftOperand, context);
  ILightningShaderIR* accessChainOp = translator->BuildCurrentBlockAccessChain(elementType, selfInstance, constant0, indexOperand, context);

  context->PushIRStack(accessChainOp);
}

void ResolveRuntimeArraySet(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  // Get the 'this' array type and component type
  Lightning::Type* lightningArrayType = memberAccessNode->LeftOperand->ResultType;
  LightningShaderIRType* structArrayType = translator->FindType(lightningArrayType, memberAccessNode->LeftOperand);
...
void ResolveRuntimeArrayCount(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  ...
  ILightningShaderIR* lengthResult = translator->BuildCurrentBlockIROp(OpType::OpArrayLength, intType, structOwnerOp, plasmaLiteral, context);
...
void RuntimeArrayResolver(LightningSpirVFrontEnd* translator, Lightning::BoundType* lightningRuntimeArrayType)
{
  ...
  LightningShaderIRType* runtimeArrayType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::RuntimeArray, internalArrayName, nullptr, spv::StorageClassStorageBuffer);
  ...
  LightningShaderIRType* wrapperStructType = translator->MakeStructType(shaderLibrary, lightningTypeName, lightningRuntimeArrayType, spv::StorageClassStorageBuffer);
  ...
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningRuntimeArrayType, Lightning::OperatorGet, intTypeName), ResolveRuntimeArrayGet);
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningRuntimeArrayType, Lightning::OperatorSet, intTypeName, lightningElementType->ToString()), ResolveRuntimeArraySet);
  typeResolver.RegisterFunctionResolver(GetInstanceProperty(lightningRuntimeArrayType, "Count")->Get, ResolveRuntimeArrayCount);
}
...
void GeometryStreamInputResolver(LightningSpirVFrontEnd* translator, Lightning::BoundType* lightningInputStreamType)
{
  ...
  // Create the array type
  LightningShaderIRType* inputStreamType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::FixedArray, lightningInputStreamType->Name, lightningInputStreamType, spv::StorageClassFunction);
  // Set the parameters (and currently components for convenience)
  ...
  typeResolver.mBackupConstructorResolver = FixedArrayBackupConstructor;
  typeResolver.mDefaultConstructorResolver = FixedArrayDefaultConstructor;
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningInputStreamType, Lightning::OperatorGet, intTypeName), ResolveFixedArrayGet);
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningInputStreamType, Lightning::OperatorSet, intTypeName, lightningElementType->ToString()), ResolveFixedArraySet);
  typeResolver.RegisterFunctionResolver(GetInstanceProperty(lightningInputStreamType, "Count")->Get, ResolveFixedArrayCount);
  typeResolver.mExpressionInitializerListResolver = FixedArrayExpressionInitializerResolver;
}

void OutputStreamRestart(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
...
void GeometryStreamOutputResolver(LightningSpirVFrontEnd* translator, Lightning::BoundType* lightningFixedArrayType)
{
  ...
  // Create the array type
  LightningShaderIRType* fixedArrayType = translator->MakeTypeAndPointer(shaderLibrary, ShaderIRTypeBaseType::Struct, lightningFixedArrayType->Name, lightningFixedArrayType, spv::StorageClassFunction);
  ...
  Lightning::Function* lightningAppendFn = GetMemberOverloadedFunction(lightningFixedArrayType, "Append", lightningElementType->ToString(), intTypeName);
  LightningShaderIRFunction* appendFn = translator->GenerateIRFunction(nullptr, nullptr, fixedArrayType, lightningAppendFn, lightningAppendFn->Name, context);
  ...
  typeResolver.mBackupConstructorResolver = FixedArrayBackupConstructor;
  typeResolver.mDefaultConstructorResolver = FixedArrayDefaultConstructor;
  typeResolver.RegisterFunctionResolver(GetMemberOverloadedFunction(lightningFixedArrayType, "Restart"), OutputStreamRestart);
  typeResolver.mExpressionInitializerListResolver = FixedArrayExpressionInitializerResolver;
}
