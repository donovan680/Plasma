...
bool LightningShaderIRCompositor::Composite(ShaderDefinition& shaderDef, const ShaderCapabilities& capabilities, LightningShaderSpirVSettingsRef& settings)
{
  mSettings = settings;
...
void LightningShaderIRCompositor::CollectFragmentsPerStage(Array<LightningShaderIRType*>& inputFragments, CompositedShaderInfo& compositeInfo)
{
  // Reset all stages
  ...
  if(!geometryStageInfo.mPrimitiveTypes.Empty())
  {
    ...
    Lightning::GeometryFragmentUserData* geometryUserData = geometryFragmentType->mLightningType->Has<Lightning::GeometryFragmentUserData>();
    ...
    for(size_t i = 0; i < stageInfo->mFragmentTypes.Size(); ++i)
    {
      ...
      if(!fragmentType->mHasMainFunction)
      {
        ...
        if(stageInfo->mFragmentType == FragmentType::Geometry)
          msg = String::Format("Geometry fragment '%s' must have a 'Main' function of signature(InputStream, OutputStream).", lightningType->Name.c_str());
        else
...
void LightningShaderIRCompositor::CollectExpectedOutputs(Array<LightningShaderIRType*>& fragmentTypes, StageAttachmentLinkingInfo& linkingInfo)
{
...
void LightningShaderIRCompositor::Link(StageAttachmentLinkingInfo& prevStageInfo, StageLinkingInfo* currentStage, Array<LightningShaderIRType*>& fragmentTypes, StageAttachmentLinkingInfo& currStageInfo)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
    ...
    for(size_t fieldIndex = 0; fieldIndex < fragmentTypeMeta->mFields.Size(); ++fieldIndex)
    {
      ...
      for(size_t attributeIndex = 0; attributeIndex < fieldMeta->mAttributes.Size(); ++attributeIndex)
      {
        ...
        if(attribute->mAttributeName == nameSettings.mPropertyInputAttribute)
        {
          ...
          fieldLinkInfo.mFieldPropertyName = MakePropertyName(fieldLinkInfo.mOutputFieldDependencyName, fieldMeta->mOwner->mLightningName);
          break;
        }
...
void LightningShaderIRCompositor::AddStageInput(ExpectedOutputData* previousStageOutputData, StageAttachmentLinkingInfo* currStageAttachment, StringParam fieldVarName, StringParam fieldAttributeName)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
ShaderIRFieldMeta* LightningShaderIRCompositor::FindHardwareBuiltInInput(ShaderFieldKey& fieldKey, FragmentType::Enum fragmentType)
{
  LightningShaderSpirVSettings* settings = mSettings;
...
ShaderIRFieldMeta* LightningShaderIRCompositor::FindHardwareBuiltInOutput(ShaderFieldKey& fieldKey, FragmentType::Enum fragmentType)
{
  LightningShaderSpirVSettings* settings = mSettings;
  BuiltInStageDescription& builtInStageDescription = settings->mBuiltIns[fragmentType];
...
void LightningShaderIRCompositor::GetStageFieldName(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute, String& fieldVarNameOut, String& fieldAttributeNameOut)
{
...
void LightningShaderIRCompositor::ResolveGpuStage(CompositedShaderInfo& compositeInfo)
{
  ...
  String renderTargetType = LightningTypeId(Lightning::Real4)->ToString();
...
void LightningShaderIRCompositor::GenerateLightningComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
...
void LightningShaderIRCompositor::GenerateBasicLightningComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ...
  for(; !outputRange.Empty(); outputRange.PopFront())
  {
    ...
    builder << MakeFragmentVarName(fieldMeta->mOwner) << "." << fieldMeta->mLightningName << ";" << builder.EmitLineReturn();
  }
...
void LightningShaderIRCompositor::GenerateGeometryLightningComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
  ShaderCodeBuilder builder;
...
void LightningShaderIRCompositor::GenerateComputeLightningComposite(StageLinkingInfo* currentStage, ShaderStageDescription& stageResults, ShaderIRAttributeList& extraAttributes)
{
  ...
  Lightning::ComputeFragmentUserData* computeUserData = computeFragmentType->mLightningType->Has<Lightning::ComputeFragmentUserData>();
...
void LightningShaderIRCompositor::CreateFragmentAndCopyInputs(StageLinkingInfo* currentStage, ShaderCodeBuilder& builder, StringParam currentClassName, LightningShaderIRType* fragmentType, StringParam fragmentVarName)
{
  FragmentLinkingInfo& linkInfo = currentStage->mFragmentLinkInfoMap[fragmentType];
...
void LightningShaderIRCompositor::DeclareFieldsInOrder(ShaderCodeBuilder& builder, StageAttachmentLinkingInfo& linkingInfo, OrderedHashSet<ShaderFieldKey>& orderMap)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void LightningShaderIRCompositor::DeclareFieldsWithAttribute(ShaderCodeBuilder& builder, StageAttachmentLinkingInfo& linkingInfo, OrderedHashSet<ShaderFieldKey>& fieldSet, StringParam attributeName)
{
  SpirVNameSettings& nameSettings = mSettings->mNameSettings;
...
void LightningShaderIRCompositor::GenerateStageDescriptions(CompositedShaderInfo& compositeInfo, ShaderDefinition& shaderDef)
{
...
void LightningShaderIRCompositor::FragmentLinkingInfo::CopyFieldTo(FieldLinkingInfo& fieldInfo, ShaderFragmentDescription& fragDesc)
{
...
LightningShaderIRCompositor::ResolvedFieldOutputInfo::ResolvedFieldOutputInfo(StringParam fieldName, ShaderIRFieldMeta* outputDependency)
{
...
void LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedField(ShaderIRFieldMeta* fieldMeta, StringParam attributeName)
{
  AddResolvedField(fieldMeta->mLightningName, fieldMeta->mLightningType->ToString(), attributeName);
}

void LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedFieldProperty(ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute)
{
...
void LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedStageField(SpirVNameSettings& nameSettings, ShaderIRFieldMeta* fieldMeta, StringParam fieldName, StringParam attributeName, StringParam attributeParameter)
{
...
ShaderIRAttribute* LightningShaderIRCompositor::StageAttachmentLinkingInfo::AddResolvedField(StringParam fieldName, StringParam fieldType, StringParam attributeName)
{
...
LightningShaderIRCompositor::ResolvedFieldInfo* LightningShaderIRCompositor::StageAttachmentLinkingInfo::CreateResolvedField(StringParam fieldName, StringParam fieldType)
{
