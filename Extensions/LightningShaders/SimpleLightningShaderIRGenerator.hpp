...
/// generator will convert lightning shaders to spir-v binary, run this binary through the
...
public:
  ...
  void CreateReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults);
  ...
  void FindSampledImageReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName, Array<ShaderResourceReflectionData*>& results);
  /// Finds all potential images and sampled images that the given Image property results in.
  void FindImageReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName, Array<ShaderResourceReflectionData*>& results);
  /// Finds all potential samplers and sampled images that the given Sampler property results in.
  void FindSamplerReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName, Array<ShaderResourceReflectionData*>& results);
...
private:
  ...
  void CreateUniformReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults);

  // Creates the final mapped data for samplers, images, and sampled images.
  void CreateSamplerAndImageReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults);
  ...
  void CreateSimpleOpaqueTypeReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults);
};
...
/// This should give simple examples of how to use the lightning shader translation system.
