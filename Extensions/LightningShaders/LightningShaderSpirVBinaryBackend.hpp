...
public:
  ...
  int FindId(ILightningShaderIR* instruction, bool assertOnZero = true);
...
public:
  ...
  void TranslateType(LightningShaderIRType* type, ShaderStreamWriter& writer, ShaderStageInterfaceReflection& reflectionData);
...
private:
  ...
  void TranslateLibrary(LightningShaderIRLibrary* library, ShaderStreamWriter& writer, ShaderStageInterfaceReflection& reflectionData);
  ...
  void GenerateDummyMain(LightningShaderIRType* type, LightningShaderIRLibrary* library, TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  void GenerateGlobalsInitializerFunction(TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  void RegisterLateBoundFunctions(LateBoundFunctionMap& lateBoundFunctionMap, TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  void Clear();

  void AddDecorationCapabilities(TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  void AddDecorationCapabilities(EntryPointInfo* entryPoint, TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  void AddDecorationCapabilities(LightningShaderIROp* decorationOp, TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  void AddMemberDecorationCapabilities(LightningShaderIROp* memberDecorationOp, TypeDependencyCollector& collector, LightningShaderToSpirVContext* context);
  ...
  void WriteSpecializationConstantBindingDecorations(TypeDependencyCollector& typeCollector, LightningShaderToSpirVContext* context);
  ...
  void WriteBlockInstructions(BasicBlock* block, Array<ILightningShaderIR*>& instructions, LightningShaderToSpirVContext* context);
