...
LightningShaderIROp* ShaderInterfaceStruct::GetFieldPointerByIndex(size_t index, EntryPointGeneration* entryPointGeneration, BasicBlock* block, spv::StorageClass storageClass)
{
  return GetFieldPointerByIndex(mInstance, index, entryPointGeneration, block, storageClass);
}

LightningShaderIROp* ShaderInterfaceStruct::GetFieldPointerByIndex(LightningShaderIROp* instance, size_t index, EntryPointGeneration* entryPointGeneration, BasicBlock* block, spv::StorageClass storageClass)
{
...
void ShaderInterfaceStruct::DeclareInterfaceType(EntryPointGeneration* entryPointGeneration, InterfaceInfoGroup& interfaceGroup, EntryPointInfo* entryPointInfo)
{
  ...
  LightningShaderIRType* blockType = translator->MakeStructType(translator->mLibrary, blockTypeName, nullptr, interfaceGroup.mStorageClass);
...
void ShaderInterfaceStruct::DefineInterfaceType(EntryPointGeneration* entryPointGeneration, InterfaceInfoGroup& interfaceGroup, EntryPointInfo* entryPointInfo)
{
  ...
  LightningShaderIROp* interfaceInstance = translator->BuildOpVariable(&entryPointInfo->mVariables, interfaceType->mPointerType, interfaceGroup.mStorageClass, context);
...
void ShaderInterfaceStruct::CopyInterfaceType(EntryPointGeneration* entryPointGeneration, InterfaceInfoGroup& interfaceGroup, EntryPointInfo* entryPointInfo, EntryPointHelperFunctionData& copyHelperData)
{
  ...
  for(size_t i = 0; i < mFields.Size(); ++i)
  {
    ...
    for(size_t j = 0; j < fieldInfo.mLinkedFields.Size(); ++j)
    {
      ...
      LightningShaderIROp* blockAccessChainOp = translator->BuildIROp(targetFnBlock, OpType::OpAccessChain, blockFieldMemberType, blockVarOp, blockIndexConstant, context);
      ...
      LightningShaderIROp* selfAccessChainOp = translator->BuildIROp(targetFnBlock, OpType::OpAccessChain, memberPtrType, copyHelperData.mSelfParam, selfIndexConstant, context);
...
LightningShaderIROp* ShaderInterfaceGlobals::GetFieldPointerByIndex(size_t index, EntryPointGeneration* entryPointGeneration, BasicBlock* block, spv::StorageClass storageClass)
{
...
void ShaderInterfaceGlobals::DefineInterfaceType(EntryPointGeneration* entryPointGeneration, InterfaceInfoGroup& interfaceGroup, EntryPointInfo* entryPointInfo)
{
  ...
  for(size_t i = 0; i < fieldList.Size(); ++i)
  {
    ...
    LightningShaderIROp* varOp = translator->BuildOpVariable(&entryPointInfo->mVariables, interfaceFieldMemberType, interfaceGroup.mStorageClass, context);
...
void ShaderInterfaceGlobals::CopyInterfaceType(EntryPointGeneration* entryPointGeneration, InterfaceInfoGroup& interfaceGroup, EntryPointInfo* entryPointInfo, EntryPointHelperFunctionData& copyHelperData)
{
  ...
  for(size_t i = 0; i < mFields.Size(); ++i)
  {
    ...
    for(size_t j = 0; j < fieldInfo.mLinkedFields.Size(); ++j)
    {
      ...
      // Get a pointer to the field on 'this'
      LightningShaderIROp* selfIndexConstant = translator->GetIntegerConstant(memberIndex, context);
      LightningShaderIROp* selfAccessChainOp = translator->BuildIROp(targetFnBlock, OpType::OpAccessChain, memberPtrType, functionData.mSelfParam, selfIndexConstant, context);
...
LightningShaderIROp* ShaderInterfaceStructArray::GetPointerByIndex(ILightningShaderIR* index, ShaderFieldKey& fieldKey, EntryPointGeneration* entryPointGeneration, BasicBlock* block, spv::StorageClass storageClass)
{
  LightningShaderIRType* shaderSubType = mStructType->mType;
  LightningShaderIROp* itemInstance = entryPointGeneration->mTranslator->BuildIROp(block, OpType::OpAccessChain, shaderSubType->mPointerType, mInstance, index, entryPointGeneration->mContext);
...
//-------------------------------------------------------------------GeometryStageInfo
GeometryStageInfo::GeometryStageInfo(LightningSpirVFrontEnd* translator, LightningShaderIRType* shaderType, Lightning::GenericFunctionNode* node)
{
...
void EntryPointGeneration::DeclareVertexInterface(LightningSpirVFrontEnd* translator, Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningSpirVFrontEndContext* context)
{
  Clear();
...
void EntryPointGeneration::DeclareGeometryInterface(LightningSpirVFrontEnd* translator, Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningSpirVFrontEndContext* context)
{
  Clear();
  ...
  LightningShaderIRConstantLiteral* modeLiteral = translator->GetOrCreateConstantIntegerLiteral(spv::ExecutionModeOutputVertices);
...
void EntryPointGeneration::DeclarePixelInterface(LightningSpirVFrontEnd* translator, Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningSpirVFrontEndContext* context)
{
  Clear();
...
void EntryPointGeneration::DeclareComputeInterface(LightningSpirVFrontEnd* translator, Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningSpirVFrontEndContext* context)
{
  Clear();
  ...
  LightningShaderIROp* executionModeOp = mTranslator->BuildIROp(block, OpType::OpExecutionMode, nullptr, entryPointInfo->mEntryPointFn, context);
...
void EntryPointGeneration::CreateEntryPointFunction(Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningShaderIROp*& selfVarOp, BasicBlock*& entryPointBlock)
{
  ...
  LightningShaderIROp* constructorOp = translator->BuildIROp(entryPointBlock, OpType::OpFunctionCall, defaultConstructorReturnType, defaultConstructorFn, selfVarOp, context);
}

void EntryPointGeneration::BuildBasicEntryPoint(Lightning::GenericFunctionNode* node, LightningShaderIRFunction* function, LightningShaderIRFunction* copyInputsFn, LightningShaderIRFunction* copyOutputsFn)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  LightningShaderIRType* voidType = translator->FindType(LightningTypeId(void), node);
...
EntryPointHelperFunctionData EntryPointGeneration::GenerateCopyHelper(LightningShaderIRFunction* userMainFn, StringParam name)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  ...
  LightningShaderIROp* selfParam = translator->BuildIROp(&helperFn->mParameterBlock, OpType::OpFunctionParameter, currentType->mPointerType, context);
...
EntryPointHelperFunctionData EntryPointGeneration::GenerateGeometryCopyHelper(LightningShaderIRFunction* userMainFn, StringParam name, LightningShaderIRType* inputStreamType, LightningShaderIROp*& inputStreamVar)
{
...
void EntryPointGeneration::WriteGeometryStageInterface(LightningShaderIRFunction* function, GeometryStageInfo& stageInfo, EntryPointInfo* entryPointInfo, EntryPointHelperFunctionData& copyInputsData, LightningShaderIROp* copyInputsStreamVar)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
...
void EntryPointGeneration::CollectGeometryStreamTypes(LightningShaderIRFunction* function, GeometryStageInfo& stageInfo)
{
  ...
  for(size_t i = 0; i < function->mBlocks.Size(); ++i)
  {
    ...
    for(size_t j = 0; j < block->mLocalVariables.Size(); ++j)
    {
      ...
      Lightning::GeometryStreamUserData* streamUserData = streamPointerType->mLightningType->Has<Lightning::GeometryStreamUserData>();
...
void EntryPointGeneration::DeclareGeometryVertexInputs(GeometryStageInfo& stageInfo, EntryPointInfo* entryPointInfo, ShaderInterfaceInfo& vertexInputInterfaceInfo, EntryPointHelperFunctionData& copyInputsData, LightningShaderIROp* copyInputsStreamVar, Array<ShaderInterfaceType*>& inputStreamInterfaceTypes, Array<ShaderInterfaceType*>& inputVertexInterfaceTypes)
{
  BasicBlock* copyInputsBlock = copyInputsData.mBlock;
...
ShaderInterfaceStructArray* EntryPointGeneration::DeclareGeometryVertexInput(InterfaceInfoGroup& interfaceGroup, EntryPointInfo* entryPointInfo, GeometryInOutTypeInfo& info, Array<ShaderInterfaceType*>& inputStreamInterfaceTypes, Array<ShaderInterfaceType*>& inputVertexInterfaceTypes)
{
  ...
  LightningShaderIRType* arrayType = translator->MakeTypeAndPointer(translator->mLibrary, ShaderIRTypeBaseType::FixedArray, info.mArrayTypeName, nullptr, interfaceGroup.mStorageClass);
...
void EntryPointGeneration::GenerateProvokingVertexAppend(GeometryStageInfo& stageInfo, EntryPointInfo* entryPointInfo, GeometryAppendFunctionData& appendFnData, LightningShaderIRType* outputStreamType)
{
  ...
  Lightning::Function* lightningAppendFn = GetMemberOverloadedFunction(lightningStreamType, "Append", outputVertexType->mName, "Integer");
  ...
  LightningShaderIROp* outVertexInstance = translator->BuildOpVariable(firstBlock, outVertexPtrType, spv::StorageClassFunction, context);
...
LightningShaderIRFunction* EntryPointGeneration::CloneAppendFn(LightningShaderIRFunction* originalAppendFn)
{
  ...
  for(size_t i = 0; i < originalParameterBlock->mLines.Size(); ++i)
  {
    ...
    LightningShaderIROp* newParameterOp = translator->BuildIROp(newParameterBlock, OpType::OpFunctionParameter, originalParameterOp->mResultType, context);
  ...
  for(size_t i = 0; i < count; ++i)
  {
    ...
    for(size_t j = 0; j < fieldMeta->mAttributes.Size(); ++j)
    {
      ...
      if(!fieldName.Empty())
      {
        ...
        LightningShaderIROp* source = GetNamedMemberInstanceFrom(block, appendFnData.mOutputDataInstance, fieldName, spv::StorageClassFunction);
      ...
      else
      {
        ...
        for(size_t j = 0; j < inputStreamInterfaceTypes.Size(); ++j)
        {
          ...
          if(inputStreamType->ContainsField(fieldKey))
          {
            ...
            LightningShaderIROp* inputInstance = inputStreamType->GetPointerByIndex(appendFnData.mDefaultVertexId, fieldKey, this, block, spv::StorageClassInput);
...
void EntryPointGeneration::CollectInterfaceVariables(LightningShaderIRFunction* function, ShaderInterfaceInfo& interfaceInfo, ShaderStage::Enum shaderStage)
{
  LightningSpirVFrontEndContext* context = mContext;
  LightningShaderIRType* currentType = context->mCurrentType;
...
void EntryPointGeneration::CollectInputInterfaceVariables(LightningShaderIRFunction* function, ShaderInterfaceInfo& interfaceInfo, LightningShaderIRType* type, ShaderStage::Enum shaderStage)
{
  SpirVNameSettings& nameSettings = mTranslator->mSettings->mNameSettings;
...
void EntryPointGeneration::CollectOutputInterfaceVariables(LightningShaderIRFunction* function, ShaderInterfaceInfo& interfaceInfo, LightningShaderIRType* type, ShaderStage::Enum shaderStage)
{
  SpirVNameSettings& nameSettings = mTranslator->mSettings->mNameSettings;
  ShaderIRTypeMeta* typeMeta = type->mMeta;
  for(size_t i = 0; i < typeMeta->mFields.Size(); ++i)
...
void EntryPointGeneration::CollectUniformInterfaceVariables(LightningShaderIRFunction* function, ShaderInterfaceInfo& interfaceInfo, LightningShaderIRType* type, ShaderStage::Enum shaderStage)
{
  SpirVNameSettings& nameSettings = mTranslator->mSettings->mNameSettings;
...
InterfaceInfoGroup* EntryPointGeneration::ProcessBuiltIn(LightningShaderIRFunction* function, ShaderInterfaceInfo& interfaceInfo, ShaderStage::Enum shaderStage, ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute, spv::StorageClass storageClass, StringParam name)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  LightningShaderSpirVSettings* settings = translator->mSettings;
...
InterfaceInfoGroup* EntryPointGeneration::ProcessUniformBlock(LightningShaderIRFunction* function, ShaderInterfaceInfo& interfaceInfo, ShaderIRFieldMeta* fieldMeta, ShaderIRAttribute* attribute, HashMap<ShaderFieldKey, UniformBufferDescription*>& mapping)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  LightningShaderSpirVSettings* settings = translator->mSettings;
...
void EntryPointGeneration::CopyField(BasicBlock* targetFnBlock, LightningShaderIRType* sourceLoadType, LightningShaderIROp* source, LightningShaderIROp* dest)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  ...
  if(sourceValueType->mBaseType == ShaderIRTypeBaseType::FixedArray)
  {
    ...
    LightningShaderIROp* sourceValue = translator->BuildIROp(targetFnBlock, OpType::OpLoad, sourceValueType, source, context);
    for(size_t i = 0; i < sourceValueType->mComponents; ++i)
    {
      ...
      LightningShaderIROp* sourceElement = translator->BuildIROp(targetFnBlock, OpType::OpCompositeExtract, sourceElementType, sourceValue, indexLiteral, context);
      // Get the destination element (by pointer)
      LightningShaderIROp* indexConstant = translator->GetIntegerConstant(i, context);
      LightningShaderIROp* destElement = translator->BuildIROp(targetFnBlock, OpType::OpAccessChain, destElementType->mPointerType, dest, indexConstant, context);
  ...
  else
  {
    ...
    if(sourceBaseType == ShaderIRTypeBaseType::Int && destBaseType == ShaderIRTypeBaseType::Bool)
    {
      ...
      LightningShaderIROp* result = translator->GenerateIntegerToBoolCast(targetFnBlock, sourceValue, destValueType, context);
    ...
    if(sourceBaseType == ShaderIRTypeBaseType::Bool && destBaseType == ShaderIRTypeBaseType::Int)
    {
      ...
      LightningShaderIROp* sourceValue = translator->GetOrGenerateValueTypeFromIR(targetFnBlock, source, context);
      LightningShaderIROp* result = translator->GenerateBoolToIntegerCast(targetFnBlock, sourceValue, destValueType, context);
      translator->BuildStoreOp(targetFnBlock, dest, result, context);
      return;
    }
...
void EntryPointGeneration::CopyFromInterfaceType(BasicBlock* block, LightningShaderIROp* dest, LightningShaderIROp* source, ShaderInterfaceType* sourceInterface, spv::StorageClass destStorageClass, spv::StorageClass sourceStorageClass)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  SpirVNameSettings& nameSettings = mTranslator->mSettings->mNameSettings;
  ...
  if(sourceBaseType == ShaderIRTypeBaseType::FixedArray)
  {
    ...
    for(int index = 0; index < sourceSize; ++index)
    {
      ...
      LightningShaderIROp* sourceItem = translator->BuildIROp(block, OpType::OpAccessChain, sourceArrayItemType->mPointerType, source, indexConstant, context);
      LightningShaderIROp* destItem = translator->BuildIROp(block, OpType::OpAccessChain, destArrayItemType->mPointerType, dest, indexConstant, context);
  ...
  else if(sourceBaseType == ShaderIRTypeBaseType::Struct)
  {
    ...
    for(size_t i = 0; i < fieldCount; ++i)
    {
      ...
      for(size_t j = 0; j < interfaceField->mLinkedFields.Size(); ++j)
      {
        ...
        LightningShaderIROp* destMemberPtr = GetNamedMemberInstanceFrom(block, dest, linkedFieldMeta->mLightningName, destStorageClass);
        ...
        LightningShaderIROp* sourceMemberPtr = structInterface->GetFieldPointerByIndex(source, i, this, block, sourceStorageClass);
...
LightningShaderIROp* EntryPointGeneration::GetMemberInstanceFrom(BasicBlock* block, LightningShaderIROp* source, int sourceOffset, spv::StorageClass sourceStorageClass)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  ...
  LightningShaderIROp* sourceMemberPtr = translator->BuildIROp(block, OpType::OpAccessChain, sourceMemberPtrType, source, sourceOffsetConstant, context);
  return sourceMemberPtr;
}

LightningShaderIROp* EntryPointGeneration::GetNamedMemberInstanceFrom(BasicBlock* block, LightningShaderIROp* source, StringParam memberName, spv::StorageClass sourceStorageClass)
{
...
LightningShaderIROp* EntryPointGeneration::GetNamedMemberInstanceFrom(BasicBlock* block, LightningShaderIROp* source, const ShaderFieldKey& fieldKey, spv::StorageClass sourceStorageClass)
{
  ErrorIf(!source->mResultType->IsPointerType(), "Source must be a pointer type");
...
void EntryPointGeneration::AddMemberTypeDecorations(LightningShaderIRType* memberType, InterfaceInfoGroup::FieldInfo& fieldInfo, ShaderResourceReflectionData& memberReflection)
{
...
void EntryPointGeneration::WriteTypeDecorations(Array<InterfaceInfoGroup::DecorationParam>& decorations, BasicBlock* decorationBlock, ILightningShaderIR* toDecorate)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  ...
  for(size_t i = 0; i < decorations.Size(); ++i)
  {
    ...
    LightningShaderIRConstantLiteral* builtInDecoration = translator->GetOrCreateConstantIntegerLiteral(decoration.mDecorationType);
    LightningShaderIROp* decorationOp = translator->BuildIROp(decorationBlock, OpType::OpDecorate, nullptr, toDecorate, builtInDecoration, context);

    // If the decoration has additional parameters then write them out (limit 1 for now)
    if(decoration.mValue >= 0)
    {
      LightningShaderIRConstantLiteral* builtInKindLiteral = translator->GetOrCreateConstantIntegerLiteral(decoration.mValue);
      decorationOp->mArguments.PushBack(builtInKindLiteral);
    }
  }
}

void EntryPointGeneration::WriteMemberDecorations(Array<InterfaceInfoGroup::DecorationParam>& decorations, BasicBlock* decorationBlock, ILightningShaderIR* toDecorate, LightningShaderIRConstantLiteral* memberIndexLiteral)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  ...
  for(size_t i = 0; i < decorations.Size(); ++i)
  {
    ...
    // Write out the basic decoration
    LightningShaderIRConstantLiteral* builtInDecoration = translator->GetOrCreateConstantIntegerLiteral(decoration.mDecorationType);
    LightningShaderIROp* decorationOp = translator->BuildIROp(decorationBlock, OpType::OpMemberDecorate, nullptr, toDecorate, memberIndexLiteral, context);
...
void EntryPointGeneration::DecorateRuntimeArrays(TypeDependencyCollector& collector, EntryPointInfo* entryPointInfo)
{
  ...
  for(; !range.Empty(); range.PopFront())
  {
    ...
    AddRuntimeArrayDecorations(decorationBlock, lightningRuntimeArrayType, spirvRuntimeArrayType, elementType, stageResource);
  }
}

void EntryPointGeneration::AddRuntimeArrayDecorations(BasicBlock* decorationBlock, LightningShaderIRType* lightningRuntimeArrayType, LightningShaderIRType* spirvRuntimeArrayType, LightningShaderIRType* elementType, ShaderStageResource& stageResource)
{
  LightningSpirVFrontEnd* translator = mTranslator;
  LightningSpirVFrontEndContext* context = mContext;
  ...
  switch(elementType->mBaseType)
  {
    ...
    case ShaderIRTypeBaseType::Matrix:
    {
      ...
      // @JoshD: Maybe make a packing option for this later?
      int matrixStride = 16;
      translator->BuildMemberDecorationOp(decorationBlock, lightningRuntimeArrayType, 0, spv::DecorationMatrixStride, matrixStride, context);
...
void EntryPointGeneration::RecursivelyDecorateStructType(BasicBlock* decorationBlock, LightningShaderIRType* structType, ShaderStageResource& stageResource)
{
...
void EntryPointGeneration::CreateShaderInterfaceField(ShaderInterfaceField& interfaceField, InterfaceInfoGroup& interfaceGroup, int index)
{
  ...
  interfaceField.mFieldType = translator->FindOrCreateInterfaceType(library, actualLightningType, ShaderIRTypeBaseType::Pointer, storageClass);
  interfaceField.mFieldType->mDereferenceType = translator->FindType(actualLightningType, nullptr, true);

  // Also mark the original lightning type in case this changes due to interfaces (needed to know how to map things like bool to int)
  interfaceField.mOriginalFieldType = translator->FindOrCreateInterfaceType(library, originalLightningType, ShaderIRTypeBaseType::Pointer, storageClass);
...
{
  BasicBlock* block = &entryPointInfo->mExecutionModes;
  LightningShaderIRConstantLiteral* literalOp = mTranslator->GetOrCreateConstantIntegerLiteral(spv::ExecutionModeOriginUpperLeft);
  mTranslator->BuildIROp(block, OpType::OpExecutionMode, nullptr, entryPointInfo->mEntryPointFn, literalOp, mContext);
}

LightningShaderIROp* EntryPointGeneration::FindField(ShaderFieldKey& fieldKey, Array<ShaderInterfaceType*>& interfaces, BasicBlock* block, spv::StorageClass storageClass)
{
...
void EntryPointGeneration::PerspectiveTransformAppendVertexCallback(AppendCallbackData& callbackData, void* userData)
{
  ...
  LightningShaderIROp* apiPerspectivePosition = self->FindField(apiPerspectivePositionKey, *callbackData.mOutputVertexInterfaceTypes, block, spv::StorageClassOutput);
  ...
  LightningShaderIROp* transformMatrixOp = self->FindField(perspectiveTransformKey, self->mUniforms, block, spv::StorageClassUniform);
  ...
  if(apiPerspectivePosition != nullptr && transformMatrixOp != nullptr && index != nullptr)
  {
    ...
    LightningShaderIROp* valueMatrix = self->mTranslator->GetOrGenerateValueTypeFromIR(block, transformMatrixOp, self->mContext);
    ...
    LightningShaderIROp* result = self->mTranslator->BuildIROp(block, OpType::OpMatrixTimesVector, resultType, valueMatrix, valueVector, self->mContext);
