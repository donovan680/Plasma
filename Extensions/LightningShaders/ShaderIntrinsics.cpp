...
LightningShaderIROp* GetOrCreateLanguageSpecConstant(LightningSpirVFrontEnd* translator, void* specKey, int defaultValue, StringParam specName, LightningSpirVFrontEndContext* context)
{
...
void ResolveIsLanguage(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIROp* comparisonLanguageOp = translator->WalkAndGetValueTypeResult(functionCallNode->Arguments[0], context);
  LightningShaderIROp* result = translator->BuildCurrentBlockIROp(OpType::OpIEqual, boolType, languageSpecConst, comparisonLanguageOp, context);
...
void ResolveIsLanguageMinMaxVersion(LightningSpirVFrontEnd* translator, Lightning::FunctionCallNode* functionCallNode, Lightning::MemberAccessNode* memberAccessNode, LightningSpirVFrontEndContext* context)
{
  ...
  LightningShaderIROp* isLanguageOp = translator->BuildCurrentBlockIROp(OpType::OpIEqual, boolType, languageSpecConst, comparisonLanguageOp, context);
  // Check the minVersion <= languageVersion <= maxVersion
  LightningShaderIROp* isGreaterThanMinOp = translator->BuildCurrentBlockIROp(OpType::OpSLessThanEqual, boolType, minVersionOp, languageVersionSpecConst, context);
  LightningShaderIROp* isLessThanMaxOp = translator->BuildCurrentBlockIROp(OpType::OpSLessThanEqual, boolType, languageVersionSpecConst, maxVersionOp, context);
  // Combine the comparisons together into one bool
  LightningShaderIROp* isInVersionRange = translator->BuildCurrentBlockIROp(OpType::OpLogicalAnd, boolType, isGreaterThanMinOp, isLessThanMaxOp, context);
  LightningShaderIROp* result = translator->BuildCurrentBlockIROp(OpType::OpLogicalAnd, boolType, isLanguageOp, isInVersionRange, context);
...
void RegisterShaderIntrinsics(LightningSpirVFrontEnd* translator, LightningShaderIRLibrary* shaderLibrary)
{
  ...
  Lightning::BoundType* shaderIntrinsicsType = shaderLibrary->mLightningLibrary->BoundTypes.FindValue("ShaderIntrinsics", nullptr);
