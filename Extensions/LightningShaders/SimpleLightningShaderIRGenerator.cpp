...
void SimplifiedShaderReflectionData::CreateReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults)
{
...
ShaderResourceReflectionData* SimplifiedShaderReflectionData::FindUniformReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName)
{
...
void SimplifiedShaderReflectionData::FindSampledImageReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName, Array<ShaderResourceReflectionData*>& results)
{
...
void SimplifiedShaderReflectionData::FindImageReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName, Array<ShaderResourceReflectionData*>& results)
{
  FragmentLookup* fragmentLookup = mFragmentLookup.FindPointer(fragmentType->mMeta->mLightningName);
  if(fragmentLookup == nullptr)
    return;
...
void SimplifiedShaderReflectionData::FindSamplerReflectionData(LightningShaderIRType* fragmentType, StringParam propertyName, Array<ShaderResourceReflectionData*>& results)
{
  FragmentLookup* fragmentLookup = mFragmentLookup.FindPointer(fragmentType->mMeta->mLightningName);
  if(fragmentLookup == nullptr)
    return;
...
ShaderResourceReflectionData* SimplifiedShaderReflectionData::FindStorageImage(LightningShaderIRType* fragmentType, StringParam propertyName)
{
  FragmentLookup* fragmentLookup = mFragmentLookup.FindPointer(fragmentType->mMeta->mLightningName);
  if(fragmentLookup == nullptr)
    return nullptr;
...
ShaderResourceReflectionData* SimplifiedShaderReflectionData::FindStructedStorageBuffer(LightningShaderIRType* fragmentType, StringParam propertyName)
{
  FragmentLookup* fragmentLookup = mFragmentLookup.FindPointer(fragmentType->mMeta->mLightningName);
  if(fragmentLookup == nullptr)
    return nullptr;
...
void SimplifiedShaderReflectionData::CreateUniformReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults)
{
...
void SimplifiedShaderReflectionData::CreateSamplerAndImageReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults)
{
...
void SimplifiedShaderReflectionData::CreateSimpleOpaqueTypeReflectionData(LightningShaderIRLibrary* shaderLibrary, ShaderStageDescription& stageDef, Array<PassResultRef>& passResults)
{
...
//-------------------------------------------------------------------SimpleLightningShaderGenerator
...
SimpleLightningShaderIRGenerator::SimpleLightningShaderIRGenerator(FrontEndTranslatorType* frontEndTranslator, LightningShaderSpirVSettings* settings)
  : mFragmentProject("Fragments"), mShaderProject("Shaders")
{
  SetupEventConnections();
  Initialize(frontEndTranslator, settings);
...
bool SimpleLightningShaderIRGenerator::ComposeShader(LightningShaderIRCompositor::ShaderDefinition& shaderDef, ShaderCapabilities& capabilities)
{
...
SimplifiedShaderReflectionData* SimpleLightningShaderIRGenerator::FindSimplifiedReflectionResult(LightningShaderIRType* shaderType)
{
  ShaderTranslationResult* translationResult = mShaderResults.FindPointer(shaderType->mMeta->mLightningName);
  if(translationResult == nullptr)
    return nullptr;
...
bool SimpleLightningShaderIRGenerator::CompilePipeline(LightningShaderIRCompositor::ShaderStageDescription& stageDef, ShaderPipelineDescription& pipeline)
{
  if(stageDef.mShaderCode.Empty())
    return false;
...
bool SimpleLightningShaderIRGenerator::CompilePipeline(LightningShaderIRType* shaderType, ShaderPipelineDescription& pipeline,
...
void SimpleLightningShaderIRGenerator::RecursivelyLoadDirectory(StringParam path, LightningShaderIRProject& project)
{
  ...
  // These scripts are manually created as .lightningFrag so it should be fine for now.
