...
PlasmaDefineType(RayTraceProperties);
...
void RayTraceProperties::InitializeMeta(MetaType* meta)
{
  ...
  LightningBindFieldProperty(mAttenuation);
  LightningBindFieldProperty(mSpecularScalar);
  LightningBindFieldProperty(mSpecularPower);
  ...
  LightningBindFieldProperty(mIndexOfRefraction);
  LightningBindFieldProperty(mUseTexture);
}
