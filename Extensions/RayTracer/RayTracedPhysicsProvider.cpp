...
namespace Plasma
{
PlasmaDefineType(RayTracedPhysicsProvider);
...
void RayTracedPhysicsProvider::InitializeMeta(MetaType* meta)
{
  ...
  LightningBindFieldProperty(mBasicLighting);
  LightningBindFieldProperty(mDrawTransparent);
  LightningBindFieldProperty(mObjectColorMode);  //METAREFACTOR enum
}
