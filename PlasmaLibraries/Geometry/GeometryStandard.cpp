...
LightningDefineExternalBaseType(Ray, TypeCopyMode::ValueType, builder, type)
{
  ...
  LightningFullBindMethod(builder, type, &Ray::GetPoint, LightningNoOverload, "GetPoint", "tValue")
    ->Description = LightningDocumentString("Returns the point at the given t-value.");
  LightningFullBindMethod(builder, type, &Ray::GetTValue, LightningNoOverload, "GetTValue", "point")
    ->Description = LightningDocumentString("Returns the t-value that would result in the given point projected onto the ray.");
...
LightningDefineExternalBaseType(Segment, TypeCopyMode::ValueType, builder, type)
{
  ...
  LightningFullBindMethod(builder, type, &Segment::GetPoint, LightningNoOverload, "GetPoint", "tValue")
    ->Description = LightningDocumentString("Returns the point at the given t-value.");
  LightningFullBindMethod(builder, type, &Segment::GetTValue, LightningNoOverload, "GetTValue", "point")
    ->Description = LightningDocumentString("Returns the t-value that would result in the given point projected onto the segment.");
...
LightningDefineExternalBaseType(Aabb, TypeCopyMode::ValueType, builder, type)
{
  ...
  LightningBindOverloadedMethodAs(Expand, LightningStaticOverload(Aabb, const Aabb&, Vec3Param), "Expand")
    ->Description = LightningDocumentString("Creates an aabb that contains the given aabb and point.");
  LightningBindOverloadedMethodAs(Combine, LightningStaticOverload(Aabb, const Aabb&, const Aabb&), "Expand")
    ->Description = LightningDocumentString("Creates an aabb that contains the two given aabbs.");
  LightningBindOverloadedMethodAs(Expand, LightningInstanceOverload(void, Vec3Param), "Expand")
    ->Description = LightningDocumentString("Expand this aabb to contain the given point.");
  LightningBindOverloadedMethodAs(Combine, LightningInstanceOverload(void, const Aabb&), "Expand")
    ->Description = LightningDocumentString("Expand this aabb to contain the given aabb.");
  LightningBindMethod(ContainsPoint)
    ->Description = LightningDocumentString("Does this aabb contain the given point?");
  LightningBindMethodAs(Overlap, "Overlaps")
    ->Description = LightningDocumentString("Does this aabb overlap/intersect the given aabb?");
  ...
  LightningBindMethod(SetInvalid)
    ->Description = LightningDocumentString("Sets this aabb to an invalid aabb (Real3.PositiveMax, Real3.NegativeMin)). "
    "This also makes expansion easier.");

  LightningBindGetterSetter(Extents);
  LightningBindGetterSetter(HalfExtents);
  LightningFullBindGetterSetter(builder, type,
  ...
  //LightningFullBindMethod(builder, type, &Aabb::TransformAabb, LightningConstInstanceOverload(Aabb, Mat4Param), "Transform", "transform")
  //  ->Description = LightningDocumentString("Computes the aabb of the current aabb after applying the given transformation.");
...
LightningDefineExternalBaseType(Sphere, TypeCopyMode::ValueType, builder, type)
{
  ...
  LightningBindOverloadedMethodAs(Expand, LightningStaticOverload(Sphere, const Sphere&, Vec3Param), "Expand")
    ->Description = LightningDocumentString("Creates a sphere that contains the given sphere and point.");
  LightningBindOverloadedMethodAs(Expand, LightningInstanceOverload(void, Vec3Param), "Expand")
    ->Description = LightningDocumentString("Expand this sphere to contain the given point.");
  LightningBindMethodAs(Overlap, "Overlaps")
    ->Description = LightningDocumentString("Does this sphere overlap/intersect the given sphere?");
  LightningBindGetter(Volume);
  LightningBindGetter(SurfaceArea);
...
LightningDefineExternalBaseType(Plane, TypeCopyMode::ValueType, builder, type)
{
  ...
  LightningFullBindMethod(builder, type, &LightningSelf::Set, LightningInstanceOverload(void, Vec3Param, Vec3Param), "Set", "normal point");
