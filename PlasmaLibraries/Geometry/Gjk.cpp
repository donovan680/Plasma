...
Type Gjk::TestDebug(const SupportShape *shapeA, const SupportShape *shapeB, Manifold *manifold, unsigned maxIter)
{
  ...
  while(iter_count < maxIter)
  {
    ...
    if (mSimplex.mCount)
    {
      ...
      if (proj <= sEpsilon)
      {
        // Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(support.cso, 0.02).Color(Color::Black));
        return Intersection::None;
      }
    }
...
void Gjk::DrawDebug(uint debugFlag)
{
  ...
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mCSO[i], 0.01f).Color(Color::PapayaWhip));

  // for (unsigned i = 0; i < mSimplex.mCount; ++i)
  // {
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(mSimplex.mPoints[i].cso, 0.01f).Color(Color::Blue));
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Line(mSimplex.mPoints[i].cso, mSimplex.mPoints[(i + 1) % mSimplex.mCount].cso).Color(Color::Blue));
  //   if (mSimplex.mCount == 4)
  //   {
  //     Plasma::gDebugDraw->Add(Plasma::Debug::Line(mSimplex.mPoints[i].cso, mSimplex.mPoints[(i + 2) % mSimplex.mCount].cso).Color(Color::Blue));
  //     Plasma::gDebugDraw->Add(Plasma::Debug::Line(mSimplex.mPoints[i].cso, mSimplex.mPoints[(i + 3) % mSimplex.mCount].cso).Color(Color::Blue));
  //   }
  // }
  ...
  // Plasma::gDebugDraw->Add(Plasma::Debug::Line(average, average + mSupportVector).Color(Color::Red));
}
...
void Gjk::DrawCSO(void)
{
  ...
  //   Plasma::gDebugDraw->Add(Plasma::Debug::Sphere(supports[i].cso, 0.01f).Color(Color::Green));
}
