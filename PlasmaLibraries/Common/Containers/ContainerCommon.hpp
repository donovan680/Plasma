...
/// WARNING: This should be a PlasmaSharedTemplate, however Release is optimizing out the constructor
...
struct PlasmaSharedTemplate Pair
{
  ...
  Pair(const type0& value0, MoveReference<type1> value1)
    : first(value0), second(PlasmaMove(value1))
  {
  }
...
template <typename Fn, Fn> struct PlasmaSharedTemplate static_verify_function_signature
{
  typedef Fn type;
};
...
template<typename T>
struct PlasmaSharedTemplate has_valid_compare_policy : public integral_constant<bool, has_valid_compare_policy_helper<T>::value> {};
