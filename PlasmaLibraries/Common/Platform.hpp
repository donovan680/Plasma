...
// LightningDeclareType is used on classes that sometimes derive from a base class with LightningGetDerivedType() creating a discrepancy we can't avoid
...
// We declare typedefs that may be useful for people using Lightning, but we don't use them ourselves
...
// PlasmaNoInline forces a function to not be inlined (only used to debug, and unfortunately to fix true linker bugs)
...
#else
#define PlasmaForceInline /* GCC has issues with __attribute__((always_inline)) */ inline
