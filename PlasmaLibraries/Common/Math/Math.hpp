...
///
/// \file Math.hpp
/// Central location for all the math used by the Plasma engine.
/// 
...
PlasmaShared EulerAngles ToEulerAngles(Mat3Param matrix, 
...
///of the rotations is expected to be in the given Euler angle structure.
PlasmaShared EulerAngles ToEulerAngles(Mat4Param matrix,
                                     EulerOrders::Enum order = EulerOrders::XYZs);
...
///of the rotations is expected to be in the given Euler angle structure.
PlasmaShared EulerAngles ToEulerAngles(QuatParam quaternion, 
                                     EulerOrders::Enum order = EulerOrders::XYZs);
...
PlasmaShared inline Vec4 ToVector4(Vec3Param v, real w = real(0.0)){return Vec4(v.x, v.y, v.z, w);}

// Convert from a IntVec2 to a Vec2
PlasmaShared inline Vec2 ToVec2(IntVec2 v){return Vec2(real(v.x), real(v.y));}

// Convert from a Vec2 to a IntVec2 standard float to int conversion
PlasmaShared inline IntVec2 ToIntVec2(Vec2Param vec2){return IntVec2(int(vec2.x), int(vec2.y));}
PlasmaShared inline IntVec2 ToIntVec2(Vec3Param vec3){return IntVec2(int(vec3.x), int(vec3.y));}
