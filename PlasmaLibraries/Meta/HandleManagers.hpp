...
#define DeclareReferenceCountedHandleInternals()                                                   \
  ...
  int Release()                                                                                    \
  {                                                                                                \
    ErrorIf(mPlasmaHandleReferenceCount.mCount == 0, "Invalid Release. ReferenceCount is zero.");    \
...
#define DeclareThreadSafeIdHandle(idType)                                                          \
  ...
  static ThreadLock mPlasmaHandleLock;
...
#define DefineThreadSafeIdHandle(type)                                                             \
  ...
  ThreadLock type::mPlasmaHandleLock;                                                                \
...
#define ConstructReferenceCountedHandle()                                                          \
  mPlasmaHandleReferenceCount.mCount = 0;
...
#define PlasmaBindHandle()                                                                           \
  type->HandleManager = LightningManagerId(PlasmaHandleManager<LightningSelf>);

//--------------------------------------------------------------------------------- Register Manager
// Call in the meta initialization of the library
#define PlasmaRegisterHandleManager(type)                                                            \
  LightningRegisterSharedHandleManager(PlasmaHandleManager<type>);
...
public:
  ...
  void Allocate(BoundType* type, Handle& handleToInitialize, size_t customFlags) override
  {
    ...
    if (IsSafeId<T>::value)
    {
      // METAREFACTOR - If we only ever go through LightningAllocate for objects that use this handle
      // manager, we can assign the id here and not have to deal with the raw object issue.
      // Is this something we should do? Or is that annoying to LightningAllocate everything?
                                               ...
                                               P_ENABLE_IF(IsReferenceCounted<U>::value))
  {
    ...
    // deleting itself. This will not call the lightning destructor on inherited types, causing leaks.
...
void ReferenceCounted<Base>::LightningSetupType(LL::LibraryBuilder& builder, LL::BoundType* type)
{
  PlasmaBindHandle();
}
...
template <typename idType, typename Base>
HashMap<typename SafeId<idType, Base>::HandleIdType, SafeId<idType, Base>*> SafeId<idType, Base>::mPlasmaHandleLiveObjects;

template <typename idType, typename Base>
void SafeId<idType, Base>::LightningSetupType(LL::LibraryBuilder& builder, LL::BoundType* type)
{
  PlasmaBindHandle();
}
...
template <typename idType, typename Base>
HashMap<typename ThreadSafeId<idType, Base>::HandleIdType, ThreadSafeId<idType, Base>*> ThreadSafeId<idType, Base>::mPlasmaHandleLiveObjects;
template <typename idType, typename Base>
ThreadLock ThreadSafeId<idType, Base>::mPlasmaHandleLock;

template <typename idType, typename Base>
void ThreadSafeId<idType, Base>::LightningSetupType(LL::LibraryBuilder& builder, LL::BoundType* type)
{
  PlasmaBindHandle();
}
...
//**************************************************************************************************
template <typename idType, typename Base>
typename ReferenceCountedSafeId<idType, Base>::HandleIdType ReferenceCountedSafeId<idType, Base>::mPlasmaHandleCurrentId = 1;
template <typename idType, typename Base>
HashMap<typename ReferenceCountedSafeId<idType, Base>::HandleIdType, ReferenceCountedSafeId<idType, Base>*> ReferenceCountedSafeId<idType, Base>::mPlasmaHandleLiveObjects;

template <typename idType, typename Base>
void ReferenceCountedSafeId<idType, Base>::LightningSetupType(LL::LibraryBuilder& builder, LL::BoundType* type)
{
  PlasmaBindHandle();
}
...
//**************************************************************************************************
template <typename idType, typename Base>
typename ReferenceCountedThreadSafeId<idType, Base>::HandleIdType ReferenceCountedThreadSafeId<idType, Base>::mPlasmaHandleCurrentId = 1;
template <typename idType, typename Base>
HashMap<typename ReferenceCountedThreadSafeId<idType, Base>::HandleIdType, ReferenceCountedThreadSafeId<idType, Base>*> ReferenceCountedThreadSafeId<idType, Base>::mPlasmaHandleLiveObjects;
template <typename idType, typename Base>
ThreadLock ReferenceCountedThreadSafeId<idType, Base>::mPlasmaHandleLock;

template <typename idType, typename Base>
void ReferenceCountedThreadSafeId<idType, Base>::LightningSetupType(LL::LibraryBuilder& builder, LL::BoundType* type)
{
  PlasmaBindHandle();
}
} // namespace Plasma
