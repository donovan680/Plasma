Automatic Binding
=================
Lightning comes with a set of templates/macros that allow your C++ classes and functions to be called by Lightning code. We recommend that you read up on the :ref:`LibraryBuilder` documentation first. Internally, the macros simply create a LibraryBuilder and automatically add BoundTypes, Properties, and Functions to it.
...
Redirecting To Lightning Types
--------------------------
Lightning also allows you to redirect your own types (such as your own ``Vector3`` or ``std::string``) to the Lightning equivalents. When using redirects, all functions, properties, and fields that you bind afterward will show up in Lightning as the Lightning type instead of your type. This is done via the ``LightningDeclareRedirectType`` macro which must be placed in the header and **must** be visible to all *LightningBind* macros. Similarly to other macros, you must use ``LightningDefineRedirectType`` in one cpp file.
...
.. code-block:: cpp
  ...
  // Here we tell Lightning binding that everywhere it sees Vec3 it should think of it as Real3
  LightningDeclareRedirectType(Vec3, Lightning::Real3);
...
.. code-block:: cpp
  ...
  Lightning::Real3 Vec3ToLightningReal3(const Vec3& vector)
  {
    return Lightning::Real3(vector.X, vector.Y, vector.Z);
  }
   
  Vec3 LightningReal3ToVec3(const Lightning::Real3& vector)
  {
...
.. code-block:: cpp
  ...
  // Must be seen by all places that do binding
  LightningDeclareRedirectType(std::string, Lightning::String);
...
.. code-block:: cpp
  ...
  // These can appear in any single cpp
  Lightning::String StdStringToPlasmaString(const std::string& string)
  {
    return Lightning::String(string.c_str(), string.size());
  }

  std::string PlasmaStringToStdString(const Lightning::String& string)
  {
    return std::string(string.c_str(), string.size());
  }
  LightningDefineRedirectType(std::string, StdStringToPlasmaString, PlasmaStringToStdString);
...
- Any (can accept any type in Lightning)
