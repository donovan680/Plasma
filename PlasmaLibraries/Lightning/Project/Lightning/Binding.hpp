...
namespace Lightning
{
  ...
  #define LightningStaticType(Type) LL::TypeBinding::StaticTypeId<Type>
  ...
  public:
    ...
    #define LightningVirtualTypeId(Pointer) \
      LL::TypeBinding::DiscoverDerivedType<LightningStrip(decltype(Pointer)), LL::TypeBinding::CanGetDerivedType<LightningStrip(decltype(Pointer))>::value>::Get(Pointer)
    ...
    #define LightningStrip(T) typename LL::TypeBinding::StripQualifiers<T>::Type
    ...
    #define LightningToPointer(Expression) (LL::TypeBinding::InternalToPointer<LightningStrip(decltype(Expression))>(Expression))
  ...
  // If we're in debug mode, add extra checks...
    ...
    #define LightningDeclareChecks(SelfType, BaseType)                                                                                                      \
      ...
      {                                                                                                                                                 \
        ...
        LL::CheckTypesAreRelated<SelfType, BaseType>::Test();                                                                                           \
      }                                                                                                                                                 \
...
#define LightningDeclareInheritableType(SelfType, TypeCopyMode)                 \
  ...
  LL::AutoGrabAllocatingType LightningDerivedType;                              \
  LL::BoundType* LightningDerivedTypeOverride() const                           \
  {                                                                         \
...
#define LightningInitializeTypeAs(Type, Name)                                                                                                              \
    LL::InitializeType<Type, LightningLibrary, void(*)(Type*, LL::LibraryBuilder&, LL::BoundType*)>(Name)
#define LightningInitializeType(Type)                                                                                                                     \
    LightningInitializeTypeAs(Type, #Type)

  // A helper for initializing types that belong to a library
#define LightningInitializeExternalTypeAs(Type, Name)                                                                                                     \
    LL::InitializeType<Type, LightningLibrary, void(*)(Type*, LL::LibraryBuilder&, LL::BoundType*)>(Name, LightningSetupType)
...
#define LightningDeclareDerivedTypeExplicit(SelfType, BaseType, CopyMode)                                                                                 \
    ...
    /*virtual*/ LL::BoundType* LightningGetDerivedType() const                                                                                            \
    {                                                                                                                                                 \
      return LL::GetDerivedTypeOverride<LightningSelf>(this);                                                                                             \
    }                                                                                                                                                 \
    /* This lets binding know how the class should be treated when just referred to by its name */                                                    \
    static const LL::TypeCopyMode::Enum LightningCopyMode = CopyMode;                                                                                     \
    /* This function is to be implemented by the user so that */                                                                                      \
    /* they may choose what they want to bind to reflection */                                                                                        \
    static void LightningSetupType(LL::LibraryBuilder& builder, LL::BoundType* type)
...
#define LightningDeclareBaseTypeExplicit(SelfType, CopyMode)                                                                                              \
  LightningDeclareDerivedTypeExplicit(SelfType, LL::NoType, CopyMode)
...
#define LightningDeclareType(ClassType, CopyMode)                                                                                                         \
    ...
    template <typename LightningT>                                                                                                                        \
    static LL::NoType SfinaeBase(...);                                                                                                                \
    typedef typename LP::remove_pointer<decltype(SfinaeBase<ClassType>(0))>::type LightningTempBase;                                                      \
