...
void LoadSettingsOrDefault(LightningShaderSpirVSettings& settings, StringParam defFilePath)
{
  if(!FileExists(defFilePath))
  {
    settings.mErrorSettings = LightningShaderErrorSettings();
    return;
  }
...
void LoadDirectorySettingsOrDefault(LightningShaderSpirVSettings& settings, StringParam directoryPath)
{
  String settingsFilePath = FilePath::Combine(directoryPath, "Settings.txt");
  LoadSettingsOrDefault(settings, settingsFilePath);
}

void LoadFragmentCodeDirectory(SimpleLightningShaderIRGenerator& shaderGenerator, StringParam directory)
{
