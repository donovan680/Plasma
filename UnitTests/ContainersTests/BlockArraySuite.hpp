...
{
  typedef Plasma::PodBlockArray<valueType, shiftSize> BlockArrayType;
  typedef Plasma::Array<valueType> ArrayType;
...
#define CheckArrayEqual(expected, actual, size)\
  ...
  static void Sort(CppUnitLite::TestResult& result_,const char * m_name)
  {
    ...
    Plasma::PodArray<valueType> podArray;
    for(uint i = 0; i < size; ++i)
      podArray.PushBack(Policy::GetValueAt(i));

    Plasma::Sort(actualArray.All(), Policy());
    Plasma::Sort(expectedArray.All(), Policy());
